...
const eventPayloadMap = {
  JOB_FINISHED: {
    body: (data) =>
      `Job${getJobDescriptionString(data)} finished with return code: ${data.payload.process_status}`,
    title: 'Job finished',
    actions: (data) => [
      {
  ...
  JOB_OUT_OF_RESOURCES: {
    body: (data) =>
      `Job${getJobDescriptionString(data)} out of resources${data.payload?.labels?.resource_warning ? ` (${data.payload?.labels?.resource_warning})` : ''}`,
  ...
  HC_JOB_OUT_OF_RESOURCES: {
    body: (data) =>
      `Hypercube job${getJobDescriptionString(data)} out of resources${data.payload?.labels?.resource_warning ? ` (${data.payload?.labels?.resource_warning})` : ''}`,
  ...
  JOB_DURATION_THRESHOLD: {
    body: (data) =>
      `Job${getJobDescriptionString(data)} running for more than ${formatDurationString(data.payload.threshold_level)}`,
  ...
  HC_JOB_DURATION_THRESHOLD: {
    body: (data) =>
      `Hypercube job${getJobDescriptionString(data)} running for more than ${formatDurationString(data.payload.threshold_level)}`,
  ...
  VOLUME_QUOTA_THRESHOLD: {
    body: (data) =>
      `Only ${formatDurationString(data.payload.threshold_level)} of your volume quota remain`,
...
self.addEventListener('push', (event) => {
  ...
  if (!eventPayloadMap.hasOwnProperty(notification.event)) {
    console.log(
      `No notification config set up for event of type: ${notification.event}.`,
    );
    self.registration.showNotification(notification.event, {
      icon: 'logo192.png',
...
self.addEventListener('notificationclick', (event) => {
  ...
  event.waitUntil(
    clients.matchAll({ type: 'window' }).then((clientsArr) => {
      ...
      if (!hadWindowToFocus) clients.openWindow(urlToOpen);
    }),
  );
});
