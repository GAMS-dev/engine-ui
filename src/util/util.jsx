...
const zipAsync = (filesToZip) => {
  if (!JSZip.support.blob) {
    return Promise.reject(
      new EvalError(
        'Your browser does not support zipping files. Please zip the files first and try to upload the zip archive instead.',
      ),
    );
  }
  if (filesToZip.length > 200) {
    return Promise.reject(
      new EvalError(
        'Engine UI does not support uploading more than 200 individual files! Please zip the files first and try to upload the zip archive instead.',
      ),
    );
  }
  ...
  if (accumulatedFileSize > 10e7) {
    return Promise.reject(
      new EvalError(
        'Engine UI does not support uploading individual files larger than 100MB. Please zip the files first and try uploading the zip archive instead.',
      ),
    );
  }
...
const isActiveJob = (status) =>
  status < 10 && (status === -2 || status === -10 || status >= 0);

const calcRemainingQuotaInternal = (
  quotaObj,
  quotaKey,
  usedKey,
  noQuotaVal = Infinity,
  ...
  Math.min(
    ...quotaObj.map(
      (el) => (el[quotaKey] == null ? noQuotaVal : el[quotaKey]) - el[usedKey],
    ),
  );

const calcRemainingQuota = (data, noQuotaVal = Infinity) => ({
  volume: calcRemainingQuotaInternal(
    data,
    'volume_quota',
    'volume_used',
    noQuotaVal,
...
const getQuotaWarningMessage = (quotaWarningData, quotaFormattingFn) => {
  ...
  return (
    <>
      ...
      {Number.isFinite(remainingQuota.volume) ? (
        <div>
          Remaining <span className="fst-italic">volume</span> quota:{' '}
          {remainingVolumeStr}
        </div>
      ) : (
        <></>
      )}
      {Number.isFinite(remainingQuota.disk) ? (
        <>
          Remaining <span className="fst-italic">disk</span> quota:{' '}
          {remainingDiskStr}
        </>
      ) : (
        <></>
...
const mergeSortedArrays = (arraysToMerge, comparisonFunction) => {
  const mergeInner = (arrL, arrR, comparisonFunction) => {
    ...
    while (ptrL + ptrR < arrLen) {
      if (
        ptrL < arrL.length &&
        (ptrR === arrR.length ||
          comparisonFunction(arrL[ptrL], arrR[ptrR]) <= 0)
      ) {
  ...
  while (arraysToMerge.length > 1) {
    ...
    for (let i = 0; i < arraysToMerge.length; i += 2) {
      arrTmp.push(
        mergeInner(arraysToMerge[i], arraysToMerge[i + 1], comparisonFunction),
      );
    }
...
const getInstanceData = async (server, username) => {
  const instancePoolDataPromise = axios.get(
    `${server}/usage/pools/${encodeURIComponent(username)}`,
  );
  const instanceDataPromise = axios.get(
    `${server}/usage/instances/${encodeURIComponent(username)}`,
  );
  const defaultInstanceDataPromise = axios.get(
    `${server}/usage/instances/${encodeURIComponent(username)}/default`,
  );
  ...
  const availableInstancesTmp = instanceData.data.instances_available
    .concat(
      instancePoolData.data.instance_pools_available.map((el) => {
        const newEl = el;
        newEl['is_pool'] = true;
        return newEl;
      }),
  ...
  return {
    instances: availableInstancesTmp,
    default: defaultInstanceData.data.default_instance?.label,
    rawResourceRequestsAllowed:
      instanceData.data.instances_inherited_from == null,
        ...
        instance.is_pool === true ? (
          <>
            ...
            <span style={{ paddingLeft: '5px' }}>
              {formatInstanceSpecs(
                Object.assign(instance.instance, { label: instance.label }),
                multiplierUnit,
              )}
            </span>
          </>
  ...
  if (parameterized_events?.length > 0) {
    eventsStr +=
      ...
      parameterized_events
        .map((el) => `${el.event}=${el.parameters.join(',')}`)
        .join(',');
  }
  return eventsStr;
};
