...
const getPushSubscription = async (server) => {
  ...
  if (!serviceWorkerRegistration) {
    serviceWorkerRegistration = await navigator.serviceWorker.register(
      `${server.substring(0, server.indexOf('/api'))}/webpush-service-worker.js`,
    );
  }
...
const subscribe = async (server, events, parameterizedEvents) => {
  ...
  try {
    const { serviceWorkerRegistration, pushSubscription } =
      await getPushSubscription(server);
    ...
    const webhookRespPromise = axios.post(
    ...
    throw new Error(
      `Problems subscribing to webhook. Error message: ${getResponseError(err)}`,
    );
  }
};
const unsubscribe = async (server, pushSubscription, deleteWebhook) => {
  try {
    ...
    if (err?.response?.status === 404) {
      console.error(
        'Web Push subscription could not be deleted as it was not found.',
      );
    } else {
      throw new Error(
        `Problems deleting Web Push subscription. Error message: ${getResponseError(err)}`,
      );
    }
  }
