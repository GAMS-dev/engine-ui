function getComputationTimes(data, calcStartTime, calcEndTime) {
  ...
  const dataHypercube =
    data['hypercube_job_usage'] == null ? [] : data['hypercube_job_usage'];
  ...
  let dataPoolUsageNEW = dataPoolUsage.map((pool) => ({
    ...
    deleted_at:
      pool['deleted_at'] == null ? calcEndTime : new Date(pool['deleted_at']),
  }));
  ...
  dataJobUsageNEW = dataJobUsageNEW.map((job, i) => {
    if (job['times'].length !== 0) {
      ...
      // should be in the json object
        ...
        if (debug)
          console.error(
            'Found job outside of the timeframe, even though this should not happen!',
          );
        job['times'] = 0;
      }
    }
  ...
  dataJobUsageNEW = dataJobUsageNEW.map((job) => {
    if (job['included']) {
      ...
      const index = dataPoolUsageNEW.findIndex(
        (pool) =>
          pool['pool_label'] === job['instance'] &&
          pool['created_at'] <= job['start_time'] &&
          job['start_time'] <= pool['deleted_at'],
      );
      if (index !== -1) {
  ...
  const numberUsers = new Set(
    dataJobUsageNEW.concat(dataPoolUsageNEW).map((elem) => elem.user),
  ).size;
  const numberInstances = new Set(
    dataJobUsageNEW.concat(dataPoolUsageNEW).map((elem) => elem.instance),
  ).size;

  const numberPools = new Set(dataPoolUsageNEW.map((elem) => elem.pool_label))
    .size;
