...
const UserQuotaUpdateForm = () => {
  ...
  useEffect(() => {
    const fetchRequiredData = async () => {
      try {
        const res = await axios.get(`${server}/usage/quota`, {
          params: { username: userToEdit },
        });
        setQuotaData(res.data);
        const quotasUser = res.data.filter(
          (el) => el.username === userToEdit,
        )[0];
        if (quotasUser) {
          setQuotas({
  ...
  const handleUserUpdateQuotaSubmission = async () => {
    ...
    try {
      ...
      setSubmissionErrorMsg(
        `An error occurred while updating user quotas. Error message: ${getResponseError(err)}.`,
      );
    }
  };
