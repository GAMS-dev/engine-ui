    ...
    const fetchStreamEntry = async () => {
      ...
      try {
        seReq = await axios.delete(
          isStdOut
            ? `${server}/jobs/${encodeURIComponent(token)}/unread-logs`
        ...
        if (err.response.status === 308) {
          ...
          setErrorMsg(
            `A problem occurred while retrieving the stream entry. Error message: ${getResponseError(err)}`,
          );
        }
      }
  ...
  return (
    <>
      <button
        ...
        onClick={() => setShowStreamEntry(true)}
      >
        <Activity size={18} />
      </button>
      <Modal
        show={showStreamEntry}
        onHide={() => setShowStreamEntry(false)}
        size="lg"
      >
        ...
        <Modal.Body>
          ...
          <div className="form-check">
            <input
              ...
              id="scrollToBottom"
            />
            <label className="form-check-label" htmlFor="scrollToBottom">
              Update
            </label>
          </div>
        </Modal.Body>
