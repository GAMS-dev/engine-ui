...
const WebhooksActionsButtonGroup = ({
  id,
  url,
  events,
  server,
  ...
  const handleRemoveWebhook = async () => {
    ...
    try {
      ...
      setSubmissionErrorMsg(
        `Problems deleting webhook. Error message: ${getResponseError(err)}`,
      );
    }
      ...
      {window.isSecureContext && url != null && (
        <button
          ...
          onClick={() => {
            ...
            setAlertMsg('success:URL copied to clipboard!');
          }}
        >
          Copy URL
        </button>
      ...
      >
        ...
        >
          ...
          <Modal.Body>
            ...
            <p>
              Are you sure you want to remove the webhook (url:{' '}
              <code>{url}</code>) for the event(s): <code>{events}</code>? This
              cannot be undone!
            </p>
          </Modal.Body>
          <Modal.Footer>
