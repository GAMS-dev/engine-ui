      ...
      <div className="sidebar-sticky">
        ...
        <ul className="nav sidebar-nav">
            ...
            <Link
              ...
              className={`nav-link nav-block${
                [
                  ...
                  '/webhooks',
                  '/quotas',
                ].filter((el) => pathname.startsWith(el)).length > 0
                  ? ''
                  : ' active'
              }`}
            >
          ...
          <li className="nav-item">
            <Link
              to="/models"
              className={`nav-link nav-block${['/models', '/groups', '/nsusers'].filter((el) => pathname.startsWith(el)).length > 0 ? ' active' : ''}`}
            >
          ...
          <li className="nav-item">
            <Link
              to="/users"
              className={`nav-link nav-block${['/users', '/licenses', '/usage', '/new-user'].filter((el) => pathname.startsWith(el)).length > 0 ? ' active' : ''}`}
            >
          ...
          {serverInfo.in_kubernetes === true &&
            (serverConfig.instance_pool_access === 'ENABLED' ||
              ...
              (serverConfig.instance_pool_access === 'INVITER_ONLY' &&
                roles?.includes('inviter'))) && (
              <li className="nav-item">
                <Link
                  to="/pools"
                  className={`nav-link nav-block${pathname.startsWith('/pools') ? ' active' : ''}`}
                >
          ...
          {(serverConfig.webhook_access === 'ENABLED' ||
            (roles && roles.includes('admin')) === true) && (
            <li className="nav-item">
              <Link
                to="/webhooks"
                className={`nav-link nav-block${pathname.startsWith('/webhooks') ? ' active' : ''}`}
              >
          ...
          <li className="nav-item">
            <Link
              to="/cleanup"
              className={`nav-link nav-block${pathname === '/cleanup' ? ' active' : ''}`}
            >
          ...
          {roles && roles.find((role) => role === 'admin') !== undefined && (
            <li className="nav-item">
              <Link
                to="/administration"
                className={`nav-link nav-block${pathname.startsWith('/administration') ? ' active' : ''}`}
              >
