...
const Models = () => {
  ...
  const [tabSelected, setTabSelected] = useState(
    location.pathname.startsWith('/models')
      ? 'models'
      : location.pathname.startsWith('/nsusers')
        ? 'nsusers'
        : 'groups',
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setAlertMsg(
          `Problems while retrieving namespaces. Error message: ${getResponseError(err)}.`,
        );
        return;
      }
        ...
        .map((ns) => ({
          ...
          permission:
            roles && roles.includes('admin')
              ...
              : ns.permissions
                  .filter(
                    (perm) => perm.username === username && perm.permission > 1,
                  )
      ...
      if (refSelectedNs.current) {
        const nsIdx = availableNsTmp.findIndex(
          (el) => el.name === refSelectedNs.current,
        );
        if (nsIdx === -1) {
  ...
  useEffect(() => {
    ...
    const fetchUsersData = async () => {
      ...
      try {
        ...
        setAlertMsg(
          `Problems while retrieving users in namespace. Error message: ${getResponseError(err)}.`,
        );
        setIsLoading(false);
        return;
      }
      setUsers(
        nsReq.data
          .filter((ns) => ns.name === namespace.name)[0]
          .permissions.filter((user) => user.permission > 0)
          .map((user) => {
    ...
    const fetchModelsData = async () => {
      ...
      try {
        nsReq = await axios.get(
          `${server}/namespaces/${encodeURIComponent(namespace.name)}`,
        );
      } catch (err) {
        setAlertMsg(
          `Problems while retrieving registered models. Error message: ${getResponseError(err)}.`,
  ...
  return (
    <>
      ...
      {globalIsLoading ? (
        ...
        <div className="row">
          <div className="col-md-4 col-12 mt-1 fw-bold">Namespace</div>
              ...
              <div className="btn-group me-2">
                ...
                {(namespace.permission & 2) === 2 && (
                  <>
                    ...
                    <Link
                      to={`/models/${encodeURIComponent(namespace.name)}/new`}
                    >
                      <button
                        type="button"
                        className="btn btn-sm btn-outline-primary"
