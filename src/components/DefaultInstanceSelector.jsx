...
const DefaultInstanceSelector = ({ className }) => {
  ...
  const handleChangeDefaultInstance = async (newDefaultInstance) => {
    ...
    if (
      currentDefaultInstance != null &&
      newDefaultInstance.value === currentDefaultInstance.value
    ) {
      return;
    }
    setIsSubmitting(true);
    try {
      await axios.put(`${server}/usage/instances/${username}/default`, {
        default_label: newDefaultInstance.value,
      });
      setNewDefaultInstance(newDefaultInstance);
    } catch (err) {
      setAlertMsg(
        `Some error occurred while trying to update default instance. Error message: ${getResponseError(err)}.`,
      );
    }
        ...
        <>
          <fieldset disabled={isSubmitting}>
            <div className={className}>
              <label htmlFor="defaultInstance">Default instance</label>
              <div
                className="invalid-feedback"
                style={{
                  display: currentDefaultInstance === null ? 'block' : 'none',
                }}
              >
