...
const GroupMembers = () => {
  ...
  useEffect(() => {
    const fetchUsersData = async () => {
      ...
      try {
        ...
        setAlertMsg(
          `Problems while loading users. Error message: ${getResponseError(err)}.`,
        );
        setIsLoading(false);
        return;
      }
      if (uReq?.data?.length > 0) {
        const usersTmp = uReq.data.map((user) => user.username);
  ...
  useEffect(() => {
    const fetchGroupMembers = async () => {
      let gmReq;
      try {
        gmReq = await axios.get(
          `${server}/namespaces/${encodeURIComponent(namespace)}/user-groups`,
        );
      } catch (err) {
        setAlertMsg(
          `Problems while retrieving group members. Error message: ${getResponseError(err)}.`,
        );
        setIsLoading(false);
        return;
      }
  ...
  function addUserToGroup() {
    const postUserToGroup = async () => {
      try {
        ...
        setSubmissionErrorMsg(
          `Problems while adding user to group: ${label}. Error message: ${getResponseError(err)}.`,
        );
