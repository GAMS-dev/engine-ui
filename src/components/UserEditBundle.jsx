      ...
      try {
        ...
        if (err?.response?.status === 403) {
          ...
          setInvalidUserMessage(
            `You do not have permission to view information about user: ${userToEdit}.`,
          );
          setAlertMsg(`Unauthorized. Error message: ${getResponseError(err)}`);
        } else {
          setInvalidUserRequest(true);
          setInvalidUserMessage(
            `Failed to fetch user information. Error message: ${getResponseError(err)}`,
          );
          setAlertMsg(
            `Failed to fetch user information. Error message: ${getResponseError(err)}`,
          );
          ...
          <sup>
            <small>
              <span className="badge rounded-pill bg-secondary ms-1">
                {getUserRoleFromArray(userToEditRoles)}
              </span>
            </small>
          </sup>
        </h1>
              ...
              {!(isInviter && username === userToEdit) && (
                <>
                  ...
                  {userToEdit !== 'admin' && (
                    <>
                      <Nav.Item>
                        <Nav.Link eventKey="identity_provider">
