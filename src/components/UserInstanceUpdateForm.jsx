...
const UserInstanceUpdateForm = () => {
  ...
  const [userHasNoDefaultAssigned, setUserHasNoDefaultAssigned] =
    useState(false);
  const [defaultInheritedFrom, setDefaultInheritedFrom] = useState('');
  const [inviterHasInstancesAssigned, setInviterHasInstancesAssigned] =
    useState(false);
  ...
  useEffect(() => {
    const fetchRequiredData = async () => {
      try {
        const resUserInfo = await axios.get(`${server}/users/`, {
          params: {
            username: userToEdit,
          },
          headers: {
        ...
        const userToEditIsAdminTmp =
          resUserInfo.data[0].roles &&
        ...
        setCurrentInstanceLabelsUser(
          instanceDataUser.instances
            .filter((el) => el.is_pool !== true)
            .map((el) => el.label),
        );
        const instanceDataInviter =
          inviterNameTmp == null
            ? instanceDataUser
            : await getInstanceData(server, inviterNameTmp);

        const inviterHasInstancesAssignedTmp =
          instanceDataInviter.rawResourceRequestsAllowed === false;

        setInviterHasInstancesAssigned(inviterHasInstancesAssignedTmp);

        let instancesAllowedTmp = formatInstancesSelectInput(
          instanceDataInviter.instances,
        ...
        setInheritInstances(
          instanceDataUser.inheritedFrom == null ||
            instanceDataUser.inheritedFrom !== userToEdit,
        );
        ...
        if (userToEditIsAdminTmp) {
          ...
          selectedInstancesAllowedTmp = formatInstancesSelectInput(
            instanceDataUser.instances,
            userSettings.multiplierUnit,
          );
        }
        ...
        setInheritDefault(
          instanceDataUser.defaultInheritedFrom != null &&
            instanceDataUser.defaultInheritedFrom !== userToEdit,
        );
        ...
        if (instanceDataUser.default == null) {
          ...
          setDefaultInstance(
            instancesAllowedTmp.find(
              (el) => el.value === instanceDataUser.default,
            ),
          );
        }
      } catch (err) {
        setErrorMsg(
          `Problems while while retrieving user instances. Error message: ${getResponseError(err)}.`,
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const handleUserUpdateInstancesSubmission = async () => {
    ...
    if (!userToEditIsAdmin && inheritInstances) {
      try {
        await axios.delete(
          `${server}/usage/instances/${encodeURIComponent(userToEdit)}`,
        );
        setAlertMsg(
          `success:Instances of user: ${userToEdit} updated successfully`,
        );
      } catch (err) {
        setIsSubmitting(false);
        setSubmissionErrorMsg(
          `An error occurred while updating user instances. Error message: ${getResponseError(err)}.`,
        );
        return;
      }
    }
    try {
      ...
      if (
        ...
        (!inheritDefault ||
          !inheritInstances ||
          (inheritInstances && !inviterHasInstancesAssigned))
      ) {
      ...
      if (!userToEditIsAdmin && !inheritInstances) {
        requestData['labels'] = selectedInstancesAllowed.map(
      ...
      if (Object.keys(requestData).length > 0) {
        if (userToEditIsAdmin || inheritInstances) {
          ...
          await axios.put(
            `${server}/usage/instances/${encodeURIComponent(userToEdit)}/default`,
            requestData,
          );
        } else {
          await axios.put(
                ...
                <>
                  <div className="form-check mb-3">
                    ...
                    <label
                      ...
                      htmlFor="inheritInstances"
                    >
                      {inviterHasInstancesAssigned ? (
                        <>
                  ...
                  {!inheritInstances && (
                    <>
                        ...
                        <Select
                          ...
                          onChange={(selected) => {
                            ...
                            if (
                              ...
                              !selected
                                .map((el) => el.value)
                                .includes(defaultInstance.value)
                            ) {
                      ...
                      selectedInstancesAllowed.filter((instance) =>
                        currentInstanceLabelsUser.includes(instance.value),
                      ).length < currentInstanceLabelsUser.length ? (
                        <div className="form-check mt-3">
                          ...
                          <label
                            ...
                            htmlFor="deletePools"
                          >
                            Delete all pools associated with an instance that
                            the user is no longer allowed to use?
                          </label>
                        </div>
                      ) : (
                        <></>
              ...
              {(selectedInstancesAllowed.length > 0 || inheritInstances) && (
                <>
                  ...
                  {(!inheritDefault ||
                    ...
                    (inheritInstances && !inviterHasInstancesAssigned)) && (
                    <div className="mb-3">
                      <label htmlFor="instancesDefault">Default Instance</label>
                      <div
                        className="invalid-feedback text-center"
                        style={{
                          display: userHasNoDefaultAssigned ? 'block' : 'none',
                        }}
                      >
