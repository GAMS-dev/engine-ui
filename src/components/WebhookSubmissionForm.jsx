...
const WebhookSubmissionForm = () => {
  ...
  const [parameterizedEventsValid, setParameterizedEventsValid] =
    useState(true);
  ...
  const handleWebhookSubmission = async () => {
    ...
    if (events.length < 1 && parameterizedEvents.length < 1) {
      setFormErrors({
        events:
          'At least one event (non-parameterized and/or parameterized) must be selected',
      });
      return;
    }
    setFormErrors({});
    ...
    try {
      ...
      if (err?.response?.data?.errors) {
        ...
        setSubmissionErrorMsg(
          `Problems creating webhook. Error message: ${getResponseError(err)}`,
        );
      }
      ...
      >
        ...
        <div className="row">
          <div className="col-md-6 col-12">
            <fieldset disabled={isSubmitting}>
              <div className="mb-3">
                <label htmlFor="instanceLabel">Payload URL</label>
                <input
                  type="text"
                  className={
                    'form-control' +
                    (urlValid !== true || formErrors.url ? ' is-invalid' : '')
                  }
                  id="url"
                  ...
                  onChange={(e) => {
                    ...
                    try {
                      ...
                      setUrlValid(
                        ['http:', 'https:'].includes(url_validated.protocol),
                      );
                      setShowInsecureSslCheckbox(
                        url_validated.protocol === 'https:',
                      );
                    } catch {
                      setUrlValid(false);
                ...
                <small id="urlHelp" className="form-text text-muted">
                  URL of the server that receives the webhook requests. Only{' '}
                  <i>HTTP</i> and <i>HTTPS</i> protocols supported.
                </small>
              </div>
            </fieldset>
          </div>
        ...
        <div className="row">
          <div className="col-12">
            <fieldset disabled={isSubmitting}>
              <div className="mb-3">
                <label htmlFor="cpuReq">Secret token</label>
                <input
                  type="text"
                  className={
                    'form-control' + (formErrors.secret ? ' is-invalid' : '')
                  }
                  id="secret"
                ...
                <small id="secretHelp" className="form-text text-muted">
                  Used to authenticate payload of webhook via <i>HMAC-SHA256</i>
                  . HMAC is sent in the HTTP header <code>X-ENGINE-HMAC</code>.
                </small>
              </div>
              <div className="mb-3">
              ...
              <div className="mb-3">
                ...
                />
                <small id="secretHelp" className="form-text text-muted">
                  Webhooks can be sent either as{' '}
                  <code>application/x-www-form-urlencoded</code> or as{' '}
                  ...
                  <div className="form-check">
                    ...
                    <label className="form-check-label" htmlFor="recursive">
                      {roles.includes('admin')
                        ? 'Register webhook globally for all users?'
                        : 'Register webhook for all your invitees as well?'}
                    </label>
                  </div>
                )}
            </fieldset>
        ...
        <div className="mt-3">
          <SubmitButton
            isSubmitting={isSubmitting}
            isDisabled={!parameterizedEventsValid}
          >
            Add Webhook
          </SubmitButton>
        </div>
