...
const LicUpdateButton = (props) => {
  ...
  useEffect(() => {
    const fetchLicense = async () => {
      let lReq;
      try {
        lReq = await axios.get(
          `${server}/licenses/${encodeURIComponent(path)}`,
        );
      } catch (err) {
        setIsSubmitting(false);
        setSubmissionErrorMsg(
          `An error occurred while fetching license. Error message: ${getResponseError(err)}.`,
        );
        return;
      }
      let licFormatted = '';
      if (lReq.data.license) {
        if (b64enc) {
          ...
          while (liceMultiLine.length * 52 < lReq.data.license.length) {
            liceMultiLine.push(
              lReq.data.license.substring(
                liceMultiLine.length * 52,
  ...
  const updateLicense = async () => {
    ...
    try {
      ...
      setSubmissionErrorMsg(
        `An error occurred while updating license. Error message: ${getResponseError(err)}.`,
      );
      setIsSubmitting(false);
      return;
    }
      ...
      <Modal show={showDialog} onHide={() => setShowDialog(false)}>
        ...
        >
          ...
          <Modal.Body>
            ...
            {settings.id === 'engineLicense' && usi != null ? (
              <small>
                Unique System Identifier (USI):{' '}
                <b className="text-monospace">{usi}</b>
                {window.isSecureContext && (
                  <Button
                    className="p-0"
                    variant="link"
                    ref={copyUsiButton}
                    onClick={copyUSIToClipboard}
                ...
                <Overlay
                  target={copyUsiButton}
                  show={showCopyUsiToolTip}
                  placement="top"
                >
                  {(props) => (
            ...
            <fieldset disabled={isSubmitting}>
              <div className="mb-3">
                <label htmlFor={settings.id}>{settings.desc}</label>
                <textarea
                  id={settings.id}
