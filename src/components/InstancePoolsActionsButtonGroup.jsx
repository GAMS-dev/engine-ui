...
const InstancePoolsActionsButtonGroup = (props) => {
  ...
  const handleRemoveInstance = async () => {
    ...
    try {
      ...
      setSubmissionErrorMsg(
        `Problems deleting instance ${isPool ? 'pool' : ''}. Error message: ${responseErrorMsg}`,
      );
    }
  ...
  const handleScalePool = async () => {
    ...
    try {
      ...
      setSubmissionErrorMsg(
        `Problems scaling instance pool. Error message: ${responseErrorMsg}`,
      );
    }
    setIsSubmitting(false);
  };
  ...
  return (
    <>
          ...
          <>
            {poolSizeCurrent !== poolSize ? (
              <small className="me-3">
                Scaling {poolSizeCurrent < poolSize ? 'up' : 'down'}...
              </small>
            ) : instancePoolsEnabled && hasPoolWritePerm ? (
              <button
                className="btn btn-sm btn-outline-info"
                onClick={() => setShowScalePoolDialog(true)}
              >
                Change Size
              </button>
            ) : (
      ...
      <Modal
        show={showScalePoolDialog}
        onHide={() => setShowScalePoolDialog(false)}
      >
        <form
          onSubmit={(e) => {
            e.preventDefault();
        ...
        >
          ...
          <Modal.Body>
            ...
            <fieldset disabled={isSubmitting}>
              <div className="mb-3">
                <label htmlFor="newPoolSize">New Size</label>
                <input
                  type="number"
                  id="newPoolSize"
          ...
          <Modal.Footer>
            <Button
              variant="secondary"
              onClick={() => setShowScalePoolDialog(false)}
            >
              Cancel
            </Button>
        ...
        >
          ...
          <Modal.Body>
            ...
            <p>
              ...
              {isPool ? 'pool' : ''}: <code>{label}</code>? This cannot be
              undone!
            </p>
          </Modal.Body>
          <Modal.Footer>
