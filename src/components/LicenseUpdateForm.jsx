    ...
    const fetchLicense = async () => {
      ...
      try {
        ...
        if (err.response.status === 404) {
          setLicenseErrorMsg(
            'User does not have and does not inherit any license',
          );
          setIsLoading(false);
        } else {
          setLicenseErrorMsg(
            `Problems while retrieving user license. Error message: ${getResponseError(err)}.`,
          );
          setIsLoading(false);
        }
        return;
    ...
    if (licenseAction === 'delete') {
      try {
        ...
        if (err.response.status === 404) {
          ...
          setLicenseErrorMsg(
            `An error occurred while deleting user license. Error message: ${getResponseError(err)}.`,
          );
          setIsSubmitting(false);
          return;
        }
      ...
      try {
        ...
        setLicenseErrorMsg(
          `An error occurred while updating user license. Error message: ${getResponseError(err)}.`,
        );
        setIsSubmitting(false);
        return;
      }
        ...
        <div>
          ...
          >
            <div
              ...
              style={{ display: licenseErrorMsg !== '' ? 'block' : 'none' }}
            >
              {inheritedFrom === '' ? (
                licenseErrorMsg
              ) : (
            ...
            <fieldset disabled={isSubmitting}>
              <label htmlFor="licenseBox">GAMS License for User</label>
              <textarea
                id="licenseBox"
                rows="8"
                cols="50"
