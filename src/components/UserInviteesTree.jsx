...
const TreeNode = ({
  ...
  isRootNode,
  inviterList,
  behindUserToEdit = false,
}) => {
  const [isOpen, setIsOpen] = useState(
    isRootNode || inviterList.includes(username) || behindUserToEdit,
  );
        ...
        <ul style={{ listStyleType: 'none', paddingLeft: '20px' }}>
          {userTreeData[username].map((child, index) => (
            <TreeNode
              ...
              userRole={getUserRoleFromArray(child.roles)}
              userTreeData={userTreeData}
              inviterList={inviterList}
              behindUserToEdit={
                username === userToEdit ? true : behindUserToEdit
              }
            />
          ))}
        </ul>
  ...
  useEffect(() => {
    const fetchUsersInfo = async () => {
      try {
          ...
          if (Object.hasOwn(inviterInfoTmp, rootInviterNameTmp)) {
            if (inviterInfoTmp[rootInviterNameTmp] != null) {
              ...
              if (rootInviterNameTmp !== userToEdit) {
                ...
                const rootInviterIndex = userTreeDataTmp[
                  nextInviterTmp
                ].findIndex((user) => user.username === findRootInviterTmp);
                const rootInviterObject =
                  userTreeDataTmp[nextInviterTmp][rootInviterIndex];
        ...
        let inviterRoleTmp = userInfoReq.data.find(
          (user) => user.username === rootInviterNameTmp,
        );
        inviterRoleTmp = inviterRoleTmp
          ? getUserRoleFromArray(inviterRoleTmp.roles)
          : '';
        setRootInviterRole(inviterRoleTmp);
      } catch (err) {
        setAlertMsg(
          `Failed to fetch users information. Error message: ${getResponseError(err)}`,
        );
      ...
      <div>
        {isLoading ? (
          ...
          <div className="alert alert-danger mt-3">
            <p>
              <strong>{invalidUserMessage}</strong>
            </p>
          </div>
        ) : (
          <div className="max-main-width">
            ...
            >
              <TreeNode
                username={rootInviter}
                userRole={rootInviterRole}
                userTreeData={userTreeData}
                isRootNode={true}
                inviterList={inviterList}
              />
