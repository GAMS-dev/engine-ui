  ...
  function deleteGroup() {
    const delteUserGroup = async () => {
      try {
        ...
        setAlertMsg(
          `Problems deleting user group. Error message: ${getResponseError(err)}`,
        );
        return;
      }
      setIsSubmitting(false);
            ...
            (namespace.permission & 2) === 2 && (
              <>
                ...
                >
                  ...
                  <Modal.Body>
                    Are you sure you want to delete the group: <code>{id}</code>{' '}
                    from the namespace: <code>{namespace.name}</code>? This
                    cannot be undone!
                  </Modal.Body>
                  <Modal.Footer>
                    <Button
