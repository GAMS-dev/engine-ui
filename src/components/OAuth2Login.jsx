...
const OAuth2Login = ({ server, loginConfig, setAuthToken, setErrorMsg }) => {
  useEffect(() => {
    const initiateOAuthLogin = async (config) => {
      ...
      if (config.oidc != null) {
        ...
        requestScopes = oauthConfig.scopes
          .filter((scope_object) => defaultScopes.includes(scope_object.scope))
      ...
      sessionStorage.setItem(
        ...
        JSON.stringify({
          ...
          tokenEndpoint: oauthConfig.has_web_ui_client_secret
            ? `${server}/auth/oauth2-token`
      ...
      window.location.replace(
        `${oauthConfig.authorization_endpoint}?${queryParams.join('&')} `,
      );
    };
  ...
  const oauthLogin = useCallback(
    async (authParams, code) => {
      if (code == null) {
        setErrorMsg(
          'Internal error while retrieving authentication token from OAuth provider.',
        );
        return;
      }
      let jwt = null;
      ...
      try {
        ...
        if (authParams.isOidc) {
          ...
          if (jwt == null) {
            setErrorMsg(
              'Internal error while retrieving id token from OpenID Connect provider.',
            );
            return;
          }
        } else {
        ...
        setErrorMsg(
          `Problems retrieving authentication token from OAuth provider. Error message: ${getResponseError(err)}.`,
        );
        return;
      }
