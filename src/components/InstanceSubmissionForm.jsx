...
const InstanceSubmissionForm = () => {
  ...
  useEffect(() => {
    const fetchInstances = async () => {
      ...
      try {
        ...
        setErrorMsg(
          `Problems while while retrieving instance data. Error message: ${getResponseError(err)}.`,
        );
        setIsLoading(false);
        return;
      }
      const instanceData = iReq.data.filter(
        (el) => el.label === label && el.is_pool !== true,
      );
      if (instanceData.length > 0) {
        ...
        setTolerations(
          instanceData[0].tolerations
            .map((el) => `${el.key}=${el.value}`)
            .join(','),
        );
        setNodeSelectors(
          instanceData[0].node_selectors
            .map((el) => `${el.key}=${el.value}`)
            .join(','),
        );
        const gamsLicenseAssigned =
          instanceData[0].gams_license != null &&
          instanceData[0].gams_license !== '';
        setAssignLicense(gamsLicenseAssigned);
        setGAMSLicense(
          gamsLicenseAssigned ? instanceData[0].gams_license.trim() : '',
        );
        setIsLoading(false);
      } else {
  ...
  const handleInstanceSubmission = async () => {
    ...
    try {
      ...
      setAlertMsg(
        `success:Instance ${label ? 'updated' : 'added'} successfully`,
      );
      ...
      if (err.response && err.response.data && err.response.data.errors) {
        ...
        setSubmissionErrorMsg(
          `Problems ${label ? 'updating' : 'adding'} instance.`,
        );
      } else {
        setSubmissionErrorMsg(
          `Problems ${label ? 'updating' : 'adding'} instance. Error message: ${getResponseError(err)}`,
        );
      }
  ...
  return (
    <div>
      <div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 className="h2">
          {label ? `Update Instance: '${label}'` : 'Add new Instance'}
        </h1>
      </div>
      {errorMsg ? (
        <div
          ...
          <div className="row">
            <div className="col-md-6 col-12">
              <fieldset disabled={isSubmitting}>
                ...
                <div className="mb-3">
                  <label htmlFor="cpuReq">CPU (vCPU)</label>
                  <input
                    type="number"
                    className={
                      'form-control' +
                      (formErrors.cpu_request ? ' is-invalid' : '')
                    }
                    step="any"
                    id="cpuReq"
                    min="0"
                ...
                <div className="mb-3">
                  <label htmlFor="memReq">Memory (in MiB)</label>
                  <input
                    type="number"
                    className={
                      'form-control' +
                      (formErrors.memory_request ? ' is-invalid' : '')
                    }
                    id="memReq"
                    min="0"
                    value={memReq}
                ...
                <div className="mb-3">
                  <label htmlFor="wsReq">Workspace Size (MiB)</label>
                  <input
                    type="number"
                    className={
                      'form-control' +
                      (formErrors.workspace_request ? ' is-invalid' : '')
                    }
                    id="wsReq"
                    step="any"
                    min="0"
              ...
              <fieldset disabled={isSubmitting}>
                <div className="mb-3">
                  ...
                  <input
                    ...
                    className={
                      'form-control' +
                      (formErrors.multiplier ? ' is-invalid' : '')
                    }
                    id="multiplier"
                    min="0"
                    step="any"
                ...
                <div className="mb-3">
                  <label htmlFor="multiplier_idle">
                    Idle Multiplier ({userSettings.multiplierUnit}, applied when
                    worker in instance pool is idle)
                  </label>
                  <input
                    type="number"
                    className={
                      'form-control' +
                      (formErrors.multiplier_idle ? ' is-invalid' : '')
                    }
                    id="multiplier_idle"
                    min="0"
                    step="any"
                ...
                <div className="mb-3">
                  ...
                  <input
                    ...
                    className={
                      'form-control' +
                      (formErrors.tolerations ? ' is-invalid' : '')
                    }
                    id="tolerations"
                    autoComplete="on"
                    value={tolerations}
                ...
                <div className="mb-3">
                  ...
                  <input
                    ...
                    className={
                      'form-control' +
                      (formErrors.node_selectors ? ' is-invalid' : '')
                    }
                    id="nodeSelectors"
                    autoComplete="on"
                    value={nodeSelectors}
                ...
                <div className="form-check mb-3">
                  <input
                    ...
                    id="assignLicense"
                  />
                  <label className="form-check-label" htmlFor="assignLicense">
                    Attach GAMS license to the instance (takes precedence over
                    user and system-wide licenses)?
                  </label>
                </div>
                {assignLicense && (
                  <div className="mb-3">
                    <label htmlFor="licenseBox">GAMS License</label>
                    <textarea
                      id="licenseBox"
                      rows="8"
                      cols="50"
                      className={
                        'form-control monospace no-resize' +
                        (formErrors.gams_license ? ' is-invalid' : '')
                      }
                      value={GAMSLicense}
                      style={{ fontSize: '8pt' }}
