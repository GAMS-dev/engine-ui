...
const NamespaceQuotaUpdateForm = () => {
        ...
        const fetchRequiredData = async () => {
            try {
                const res = await axios
                    .get(`${server}/namespaces/${namespace}/disk/quota`);
                if (res.status !== 200) {
    ...
    const handleUpdateQuotaSubmission = async () => {
        ...
        try {
            if (quota) {
                await axios.put(`${server}/namespaces/${namespace}/disk/quota`, null, {
                    params: {
                        disk_quota: quota * 1e6
                    }
                });
            } else {
                await axios.delete(`${server}/namespaces/${namespace}/disk/quota`);
            }
