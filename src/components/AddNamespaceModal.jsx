...
const AddNamespaceModal = (props) => {
  const { showDialog, setShowDialog, handleSuccess, existingNamespaces } =
    props;
  ...
  const handleAddNamespace = () => {
    ...
    if (
      existingNamespaces &&
      existingNamespaces.find((ns) => ns === namespaceName)
    ) {
      setSubmissionErrorMsg(
    ...
    const postNamespace = async () => {
      try {
        ...
        setSubmissionErrorMsg(
          `Some error occurred while trying to add the namespace. Error message: ${getResponseError(err)}.`,
        );
      } finally {
        setIsSubmitting(false);
      }
