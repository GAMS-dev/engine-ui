...
const Cleanup = () => {
  ...
  const [displayFields] = useState([
    ...
    },
    {
      ...
      column: 'Actions',
      displayer: (token, type, username) => (
        <CleanupActionsButtonGroup
          token={token}
          username={username}
          type={type}
    ...
    const fetchVolumeQuota = async () => {
      try {
        ...
        if (!axios.isCancel(err)) {
          setAlertMsg(
            `Problems fetching volume quota. Error message: ${getResponseError(err)}`,
          );
        }
      }
    };
  ...
  useEffect(() => {
    const fetchCleanup = async () => {
      ...
      try {
        ...
        setAlertMsg(
          `Problems fetching cleanup information. Error message: ${getResponseError(err)}`,
        );
        setIsLoading(false);
        return;
      }
  ...
  const getTokensToDelete = async (daysThresholdTmp, deletedUsersOnly) => {
    ...
    if (deletedUsersOnly) {
      results_to_remove = results_to_remove.filter(
        (el) => el.user.deleted === true,
      );
    }
  ...
  const deleteData = async () => {
    ...
    if (showHousekeepingDialog) {
      ...
      if (isNaN(daysThresholdTmp)) {
        setSubmissionErrorMsg(
          'The number of days you entered is not a valid integer!',
        );
        setIsSubmitting(false);
        return;
      }
      try {
        tokensToRemove = await getTokensToDelete(
          daysThresholdTmp,
          deletedUsersOnly,
        );
      } catch (err) {
        setSubmissionErrorMsg(
          `Problems deleting dataset. Error message: ${getResponseError(err)}`,
        );
        setIsSubmitting(false);
        return;
      }
    ...
    for (let i = 0; i < Math.ceil(tokensToRemove.length / 1000); i += 1) {
      ...
      try {
        const tokensToRemoveTmp = tokensToRemove
          .splice(0, 1000)
          .map(
            (el) =>
              `${el.type === 'hypercube_result' ? 'hypercube_token' : 'token'}=${el.token}`,
          )
          .join('&');
  ...
  return (
    <div>
      ...
      <small>
        ...
        {fileSizeQuota === -1 ? (
          <ClipLoader size={12} />
        ) : isFinite(fileSizeQuota) ? (
          `/${formatFileSize(fileSizeQuota)}`
        ) : (
          ''
        )}
      </small>
      ...
      />
      <Modal
        show={showHousekeepingDialog}
        onHide={handleCloseHousekeepingDialog}
      >
        <form
          onSubmit={(e) => {
            e.preventDefault();
              ...
              <div className="mb-3">
                ...
                <div className="form-check mt-3">
                  <input
                    ...
                    id="deletedUsersOnly"
                    disabled={
                      datasets.filter((el) => el.deleted === true).length > 0
                    }
                  />
                  <label
                    className="form-check-label"
                    htmlFor="deletedUsersOnly"
                  >
                    Remove only data from deleted users?
                  </label>
                </div>
              </div>
            </fieldset>
      ...
      <Modal
        ...
        onHide={handleCloseDeleteConfirmDialog}
      >
        <Modal.Header closeButton>
        ...
        <Modal.Body>
          ...
          Are you sure you want to delete the data of job with token:{' '}
          <code>{dataToRemove.length ? dataToRemove[0].token : ''}</code> (user:{' '}
          <code>{dataToRemove.length ? dataToRemove[0].username : ''}</code>)
