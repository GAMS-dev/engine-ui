...
const UserSettingsFormGeneral = () => {
  ...
  const { selectedTablePageLength, setSelectedTablePageLength } =
    useOutletContext();

  return (
    <form>
      <div className="form-group mt-3 mb-3">
        <label htmlFor="tablePageLengthInput">Default table page length</label>
        <Select
          id="tablePageLength"
          inputId="tablePageLengthInput"
          isClearable={false}
          value={
            availableTablePageLengths.filter(
              (type) => type.value === selectedTablePageLength,
            )[0]
          }
          isSearchable={true}
      ...
      {serverInfo.in_kubernetes === true ? (
        <DefaultInstanceSelector className={'form-group mt-3 mb-3'} />
      ) : (
        <></>
      )}
    </form>
