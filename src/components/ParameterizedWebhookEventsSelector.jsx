...
const ParameterizedWebhookEventsSelector = ({
  parameterizedEvents,
  setParameterizedEvents,
  setIsValid,
  isSubmitting,
  validationErrors,
}) => {
  ...
  const allParameterizedEvents = [
    {
      value: 'VOLUME_QUOTA_THRESHOLD',
      label: 'Volume quota threshold reached',
    },
    {
      value: 'JOB_DURATION_THRESHOLD',
      label: 'Job duration threshold reached',
    },
    {
      value: 'HC_JOB_DURATION_THRESHOLD',
      label: 'Hypercube job duration threshold reached',
    },
  ];
  ...
  return (
    <>
      <div
        ...
        style={{ display: validationErrors !== '' ? 'block' : 'none' }}
      >
        {validationErrors}
      </div>
      {parameterizedEvents.map((event, idx) => {
        const eventType = event.split('=', 1)[0];
        const eventParameter = parseInt(
          event.substring(event.indexOf('=') + 1),
        );
        return (
          <div key={`parameterized_event_${idx}`} className="row mb-1">
            ...
            <div className="col-sm-5 col-11">
              <input
                ...
                min="0"
                className={
                  'form-control' + (isNaN(eventParameter) ? ' is-invalid' : '')
                }
                placeholder={`Event trigger (in ${eventType === 'VOLUME_QUOTA_THRESHOLD' ? userSettings.quotaUnit : 'h'})`}
                value={
                  isNaN(eventParameter)
                    ? ''
                    : eventType === 'VOLUME_QUOTA_THRESHOLD'
                      ? eventParameter / userSettings.quotaConversionFactor
                      : eventParameter / 3600
                }
                onChange={(el) => {
