...
const ToggleConfigOptionButton = ({ configKey }) => {
  ...
  const configOptions = {
    job_priorities_access: {
      ...
      confirmMessage: (
        <>
          <p>
            Please note that enabling/disabling the job priorities feature
            should only be done when the system is idle, i.e. no jobs are being
            submitted and no jobs are currently running. Otherwise, jobs may end
  ...
  const togglePatchConfig = async (confirmedViaDialog) => {
    if (
      ...
      configOptions[configKey].confirmRequired === true
    ) {
      setShowDialog(true);
      return;
    }
    ...
    newServerConfig[configKey] =
      serverConfig[configKey] === 'ENABLED' ? 'DISABLED' : 'ENABLED';
    try {
      ...
      if (confirmedViaDialog) {
        setSubmissionErrorMsg(
          `Problems updating configuration. Error message: ${getResponseError(err)}`,
        );
      } else {
        setAlertMsg(
          `Problems updating configuration. Error message: ${getResponseError(err)}`,
  ...
  return (
    <>
      <button
        ...
        onClick={() => togglePatchConfig(false)}
      >
        {serverConfig[configKey] === 'ENABLED' ? 'Disable' : 'Enable'}{' '}
        {configOptions[configKey].title}
      </button>
