...
const JobAccessGroupsSelector = (props) => {
  ...
  useEffect(() => {
    const loadUserGroups = async () => {
      try {
        const userGroupData = await axios.get(
        ...
        setAlertMsg(
          `Problems retrieving user groups. Error message: ${getResponseError(err)}.`,
        );
      }
    };
  ...
  return userGroupsLoaded === namespace ? (
    <div className="mb-3">
      {availableUserGroups.length === 0 ? (
        props.hideIfNoGroupsAvailable === true ? (
          <></>
        ) : (
          'No groups available'
        )
        ...
        <>
          <label htmlFor="access-groups">Select access groups</label>
          <Select
