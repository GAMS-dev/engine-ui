...
const InstancePools = () => {
  ...
  const [showInstancePoolAccessModal, setShowInstancePoolAccessModal] =
    useState(false);
  const [poolAccessIsSubmitting, setPoolAccessIsSubmitting] = useState(false);
  const [poolAccessSubmissionError, setPoolAccessSubmissionError] =
    useState('');
  const [, setAlertMsg] = useContext(AlertContext);
  const [serverConfig, setServerConfig] = useContext(ServerConfigContext);
  const [userSettings] = useContext(UserSettingsContext);

  const instancePoolsEnabled =
    serverConfig.instance_pool_access === 'ENABLED' ||
    (['INVITER_ONLY', 'ADMIN_ONLY'].includes(
      serverConfig.instance_pool_access,
    ) &&
      roles?.includes('admin') === true) ||
    (serverConfig.instance_pool_access === 'INVITER_ONLY' &&
      roles?.includes('inviter') === true);

  useEffect(() => {
    const fetchInstances = async () => {
      setIsLoading(true);
      try {
        const instancePoolData = await axios.get(
          `${server}/usage/pools/${username}`,
        );
  ...
  const setInstancePoolAccessConfig = async (accessConfig) => {
    ...
    try {
      ...
      setPoolAccessSubmissionError(
        `Problems updating instance pool access configuration. Error message: ${getResponseError(err)}`,
      );
  ...
  return (
    <div>
      <div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        ...
        <div className="btn-toolbar mb-2 mb-md-0">
          <div className="btn-group me-2">
            ...
            {roles &&
              roles.includes('admin') &&
              serverConfig.instance_pool_access !== 'DISABLED' && (
                <button
                  type="button"
                  className="btn btn-sm btn-outline-primary h-100"
      ...
      {serverConfig.instance_pool_access === 'DISABLED' ? (
        <p className="text-center">Instance pools disabled</p>
      ) : (
        <Table
          data={instancePools}
          ...
          displayFields={[
            ...
            },
            {
              ...
              column: 'Actions',
              displayer: (_, label, owner, size, currentSize, isCanceling) => (
                <InstancePoolsActionsButtonGroup
                  server={server}
                  label={label}
                  isPool={true}
                  hasPoolWritePerm={
                    roles?.includes('admin') === true ||
                    userInvitees.includes(owner.username)
                  }
                  poolSize={size}
      ...
      <Modal
        size="lg"
        show={showInstancePoolAccessModal !== false}
        onHide={() => setShowInstancePoolAccessModal(false)}
      >
        ...
        <Modal.Body>
          <div
            ...
            style={{
              display: poolAccessSubmissionError !== '' ? 'block' : 'none',
            }}
          >
        ...
        <Modal.Footer>
          <Button
            variant="secondary"
            onClick={() => setShowInstancePoolAccessModal(false)}
          >
            Cancel
          </Button>
          {serverConfig.instance_pool_access !== 'ADMIN_ONLY' && (
            <SubmitButton
              poolAccessIsSubmitting={poolAccessIsSubmitting}
              onClick={() => setInstancePoolAccessConfig('ADMIN_ONLY')}
          ...
          {serverConfig.instance_pool_access !== 'INVITER_ONLY' && (
            <SubmitButton
              poolAccessIsSubmitting={poolAccessIsSubmitting}
              onClick={() => setInstancePoolAccessConfig('INVITER_ONLY')}
              className="btn-primary"
            >
          ...
          {showInstancePoolAccessModal === 'enable' ? (
            <>
              <SubmitButton
                poolAccessIsSubmitting={poolAccessIsSubmitting}
                onClick={() => setInstancePoolAccessConfig('ENABLED')}
                className="btn-primary"
              >
                Enable for everyone
              </SubmitButton>
            ...
            <SubmitButton
              poolAccessIsSubmitting={poolAccessIsSubmitting}
              onClick={() => setInstancePoolAccessConfig('DISABLED')}
              className="btn-primary"
            >
              Disable
            </SubmitButton>
