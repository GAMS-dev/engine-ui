  ...
  function terminateJob(hardKill = false) {
    const deleteJob = async () => {
      try {
        await axios.delete(
          token.startsWith('hc:')
            ? `${server}/hypercube/${encodeURIComponent(token.substring(3))}?hard_kill=${hardKill}`
            : `${server}/jobs/${encodeURIComponent(token)}?hard_kill=${hardKill}`,
          {},
        );
      } catch (err) {
        setAlertMsg(
          `Problems terminating job. Error message: ${getResponseError(err)}`,
        );
        return;
      }
    };
