...
const JobRespInfoTable = (props) => {
  ...
  if (isHcJob) {
    ...
    solveTraceEntries = job.text_entries
      .map((el) => el.entry_name)
      .filter((v) => v.toLowerCase().endsWith('.solvetrace'));
  } else {
  ...
  const [streamEntry, setStreamEntry] = useState(
    isHcJob
      ? null
      : job.stream_entries.length
        ? job.stream_entries[0]
          ? job.stream_entries[0]
          : null
        : job.stdout_filename,
  );
    ...
    <>
      <table className="table table-sm table-fixed">
        ...
        <tbody>
          ...
          {(jobStatus >= 10 || (isHcJob && jobStatus === -3)) && (
            <tr>
              <th>Result ZIP</th>
              <td>
                {job.result_exists ? (
                  <>
                    <DownloadLink
                      url={
                        isHcJob
                          ? `${server}/hypercube/${encodeURIComponent(job.token)}/result`
                          : `${server}/jobs/${encodeURIComponent(job.token)}/result`
                      }
      ...
      <Modal
        ...
        onHide={() => setShowRemoveConfirmDialog(false)}
      >
        <Modal.Header closeButton>
          <Modal.Title>Please Confirm</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          Are you sure you want to remove the results of the job
          {isHcJob ? '' : ' as well as all text entries belonging to this job'}?
