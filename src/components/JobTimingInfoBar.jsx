...
const JobTimingInfoBar = ({
  ...
  useEffect(() => {
    ...
    const fetchJobTimes = async () => {
      ...
      try {
        ...
        setHasError(true);
        setAlertMsg(
          `A problem occurred while retrieving job progress info. Error message: ${getResponseError(err)}`,
        );
        return;
      }

      const data = isHcJob
        ? jpReq.data.hypercube_job_usage[0]
        : jpReq.data.job_usage[0];
      ...
      if (data.status === 0 && serverInfo.in_kubernetes !== true) {
        ...
        let jqReq;
        try {
          jqReq = await axios.get(
            isHcJob
              ? `${server}/hypercube/`
              : `${server}/jobs/${encodeURIComponent(token)}`,
            {
          ...
          console.error(
            `A problem occurred while retrieving job queue position. Error message: ${getResponseError(err)}`,
          );
          return;
        }
      ...
      if (isFinishedTmp) {
        totalDuration =
          (new Date(data.finished) - new Date(data.submitted)) / 1000;
      } else {
        totalDuration = (new Date() - new Date(data.submitted)) / 1000;
      }
      const timingsTmp = (
        isHcJob ? data.jobs.map((el) => el.times) : [data.times]
      ).map((jobTimes) => {
        let jobTimingTmp;
        if (jobTimes.length > 0) {
          const intervalDuration =
            (new Date(jobTimes[0].start) - new Date(data.submitted)) / 1000;
          jobTimingTmp = [
            {
              desc: 'Queued',
            ...
            jobTimes.map((interval, idx) => {
              let intervalDuration;
              if (interval.finish == null) {
                if (jobTimes.length > idx + 1) {
                  // job was restarted
                  intervalDuration =
                    (new Date(jobTimes[idx + 1].start) -
                      new Date(interval.start)) /
                    1000;
                } else if (isFinishedTmp) {
                  // job is finished, but 'finish' was not set for interval
                  intervalDuration =
                    (new Date(data.finished) - new Date(interval.start)) / 1000;
                } else {
                  // job is still running
                  intervalDuration =
                    (new Date() - new Date(interval.start)) / 1000;
                  setJobStatus(data.status);
                }
              } else {
                intervalDuration =
                  (new Date(interval.finish) - new Date(interval.start)) / 1000;
              }
              return {
                desc: 'Running',
          ...
          if (isFinishedTmp && jobTimes[jobTimes.length - 1].finish != null) {
            const intervalDuration =
              (new Date(data.finished) -
                new Date(jobTimes[jobTimes.length - 1].finish)) /
              1000;
            jobTimingTmp.push({
              desc: 'Finalizing',
  ...
  return (
    <div className="job-timings-info-wrapper">
      {timingData == null ? (
        hasError ? (
          ...
          <div key={`jobTiminBar${idx}`} className="job-timings-info-bar">
            {jobTimingData.map((timingObj, idx) => {
              ...
              const durationDisplayed =
                timingObj.duration > 3600
                  ? `${Math.round(timingObj.duration / 360) / 10}h`
                  : durationSeconds;
              return (
                <OverlayTrigger
                  key={`jobTimings${idx}`}
                ...
                >
                  <div
                    className={timingObj.className}
                    style={{
                      width: `${Math.floor(timingObj.width * 100) / 100}%`,
                    }}
                  >
