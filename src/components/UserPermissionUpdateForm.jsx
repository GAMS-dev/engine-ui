...
const UserPermissionUpdateForm = () => {
      ...
      (await Promise.allSettled(requests)).forEach((result, idx) => {
        if (result.status === 'rejected') {
          if (idx === 0) {
            ...
            setRequierDataErrorMessage(
              `Problems while retrieving namespaces. Error message: ${getResponseError(result.reason)}.`,
            );
          } else {
            setRequierDataError(true);
            setRequierDataErrorMessage(
              `Problems while retrieving user roles. Error message: ${getResponseError(result.reason)}.`,
            );
          }
          return;
        }
        ...
        if (idx === 0) {
          const nsPerm = responseData.map((el) => ({
            name: el.name,
            perm: el.permissions
              .filter((perm) => perm.username === userToEdit)
              .map((el) => el.permission)[0],
            maxPerm: 7,
          }));
          setNamespacePermissions(nsPerm);
          setCurrNamespacePermissions(
            nsPerm.map((el) => ({ name: el.name, perm: el.perm })),
          );
          return;
        }
  ...
  useEffect(() => {
    const fetchAvailableIDPs = async () => {
      ...
      try {
        setIDPLoading(true);
        const availableProvidersPromise = axios.get(
          `${server}/users/inviters-providers/${encodeURIComponent(inviterName)}`,
        );
        let selectedProvidersTmp = null;
        if (currentRole === 'inviter') {
          const currentProvidersResponse = await axios.get(
            `${server}/users/inviters-providers/${encodeURIComponent(userToEdit)}`,
          );
          selectedProvidersTmp = currentProvidersResponse.data.map(
            (provider) => ({ value: provider.name, label: provider.name }),
          );
        }
        const availableProvidersResponse = await availableProvidersPromise;
        const availableIdentityProvidersTmp =
          availableProvidersResponse.data.map((provider) => ({
            value: provider.name,
            label: provider.name,
          }));
        setAvailableIdentityProviders(availableIdentityProvidersTmp);
        setSelectedIdentityProvidersAllowed(
          selectedProvidersTmp == null
            ? availableIdentityProvidersTmp
            : selectedProvidersTmp,
        );
      } catch (err) {
        setErrorMsg(
          `Problems while retrieving identity providers. Error message: ${getResponseError(err)}.`,
        );
      } finally {
        setIDPLoading(false);
      }
  ...
  const handleUserUpdateSubmission = async () => {
    ...
    if (currentRole !== newRole) {
      if (
        newRole === 'inviter' &&
        selectedIdentityProvidersAllowed.length === 0
      ) {
        setSubmissionErrorMsg(
          'Please select at least one identity provider that the user is allowed to invite with, or select the "User" role.',
        );
      ...
      try {
        ...
        setIsSubmitting(false);
        setSubmissionErrorMsg(
          `An error occurred while updating user roles. Error message: ${getResponseError(err)}.`,
        );
        return;
      }
    }
    if (newRole === 'inviter' && selectedIdentityProvidersAllowed.length > 0) {
      try {
        ...
        await axios.put(
          `${server}/users/inviters-providers/${encodeURIComponent(userToEdit)}`,
          invitersProvidersForm,
        );
      } catch (err) {
        setIsSubmitting(false);
        setSubmissionErrorMsg(
          `An error occurred while updating available identity providers. Error message: ${getResponseError(err)}.`,
        );
        return;
      }
    }
    if (newRole !== 'admin') {
      for (let i = 0; i < namespacePermissions.length; i++) {
        ...
        if (
          nsPerm.perm == null ||
          currNamespacePermissions.findIndex(
            (el) => el.name === nsPerm.name && el.perm === nsPerm.perm,
          ) !== -1
        ) {
          continue;
        }
        ...
        try {
          await axios.put(
            `${server}/namespaces/${encodeURIComponent(nsPerm.name)}/permissions`,
            userUpdateForm,
          );
        } catch (err) {
          setSubmissionErrorMsg(
            `An error occurred while updating user permissions. Error message: ${getResponseError(err)}.`,
          );
          setIsSubmitting(false);
          return;
        }
      ...
      setCurrNamespacePermissions(
        namespacePermissions.map((el) => ({ name: el.name, perm: el.perm })),
      );
    }
    ...
    <>
      {requierDataError ? (
        <div className="alert alert-danger mt-3">
          <p>
            <strong>{requierDataErrorMessage}</strong>
          </p>
        </div>
      ) : (
        <div>
