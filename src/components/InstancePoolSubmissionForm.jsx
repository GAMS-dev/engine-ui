  ...
  useEffect(() => {
    const fetchInstances = async () => {
      ...
      try {
        ...
        const instancePoolsTmp = formatInstancesSelectInput(
          instanceData.instances.filter((el) => el.is_pool !== true),
        ...
        setErrorMsg(
          `An error occurred fetching instance pools. Error message: ${getResponseError(err)}.`,
        );
      }
      setIsLoading(false);
    };
    ...
    try {
      ...
      if (err.response && err.response.data && err.response.data.errors) {
        ...
        setSubmissionErrorMsg(
          `Problems adding instance pool. Error message: ${getResponseError(err)}`,
        );
      }
      ...
      {errorMsg ? (
        ...
        >
          ...
          <fieldset disabled={isSubmitting}>
            <div className="mb-3">
              <label htmlFor="poolLabel">Pool Label</label>
              <input
                type="text"
                className={
            ...
            <div className="mb-3">
              <label htmlFor="instanceReq">Instance</label>
              <Select
                inputId="instanceReq"
                isClearable={false}
                value={instanceReq}
            ...
            <div className="mb-3">
              <label htmlFor="sizeReq">Size</label>
              <input
                type="number"
                className={
                  'form-control' + (formErrors.size ? ' is-invalid' : '')
                }
                id="sizeReq"
                min="0"
                value={sizeReq}
