...
export const NamespacePermissionSelector = ({
  ...
  const [selectedNamespace, setSelectedNamespace] = useState(
    namespacePermissions != null && namespacePermissions.length > 0
      ? namespacePermissions.sort((a, b) =>
    ...
    const fetchGroups = async () => {
      ...
      try {
        const res = await axios.get(
          `${server}/namespaces/${encodeURIComponent(selectedNamespace)}/user-groups`,
        );
        const availableGroupsTmp = res.data.map((group) => ({
          value: group.label,
          label: group.label,
        }));
        const availableGroupLabels = res.data.map((group) => group.label);
        setAvailableGroups(availableGroupsTmp);
        const nsIdTmp = namespacePermissions.findIndex(
          (el) => el.name === selectedNamespace,
        );
        setSelectedGroups(() => {
        ...
        setAlertMsg(
          `Problems while retrieving user groups. Error message: ${getResponseError(err)}.`,
        );
        setAvailableGroups([]);
        setSelectedGroups([]);
      } finally {
              ...
              {namespacePermissions
                ...
                .map((ns) => (
                  <option key={ns.name} value={ns.name}>
                    {generateDesc(ns.name, ns.perm, ns.groups)}
                  </option>
                ))}
            </select>
          </div>
          <div>
            ...
            <OverlayTrigger
              ...
              overlay={
                <Tooltip id="tooltip">
                  <div>
                    Admin role required for namespace creation, deletion, and
                    license management.
                    <br />
            ...
            <input
              ...
              checked={
                readAccess === undefined
                  ? (namespacePermissions[0].perm & 4) !== 0
                  : readAccess
              }
              onChange={updateReadAccess}
              disabled={
                isLoading || (namespacePermissions[nsId].maxPerm & 4) === 0
              }
            />
            <label className="form-check-label" htmlFor="cbReadAccess">
              Read
            </label>
          </div>
          <div className="form-check form-check-inline">
            <input
              ...
              checked={
                writeAccess === undefined
                  ? (namespacePermissions[0].perm & 2) !== 0
                  : writeAccess
              }
              onChange={updateWriteAccess}
              disabled={
                isLoading || (namespacePermissions[nsId].maxPerm & 2) === 0
              }
            />
            <label className="form-check-label" htmlFor="cbWriteAccess">
              Write
            </label>
          </div>
          <div className="form-check form-check-inline">
            <input
              ...
              id="cbExecuteAccess"
              checked={
                execAccess === undefined
                  ? (namespacePermissions[0].perm & 1) !== 0
                  : execAccess
              }
              onChange={updateExecAccess}
              disabled={
                isLoading || (namespacePermissions[nsId].maxPerm & 1) === 0
              }
            />
            <label className="form-check-label" htmlFor="cbExecuteAccess">
              Execute
            </label>
          </div>
          {includeGroups === true && availableGroups.length ? (
            <div className="mb-3 mt-3">
              <label htmlFor="nsGroups">User Groups</label>
              <Select
                inputId="nsGroups"
