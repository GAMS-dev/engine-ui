...
const UserInvitationForm = () => {
  ...
  const [selectedInstancesAllowed, setSelectedInstancesAllowed] =
    useState(null);
  ...
  const [availableIdentityProviders, setAvailableIdentityProviders] = useState([
    { value: 'gams_engine', label: 'gams_engine' },
  ]);
  const [ldapIdentityProviders, setLdapIdentityProviders] = useState([]);
  const [
    selectedIdentityProvidersAllowed,
    setSelectedIdentityProvidersAllowed,
  ] = useState({ value: 'gams_engine', label: 'gams_engine' });
  ...
  const [showConfirmNoInstanceDialog, setShowConfirmNoInstanceDialog] =
    useState(false);
  ...
  useEffect(() => {
    const fetchRequiredData = async () => {
      const requests = [
        ...
        axios.get(
          `${server}/users/inviters-providers/${encodeURIComponent(username)}`,
        ),
      ];
      try {
      ...
      (await Promise.allSettled(requests)).forEach((result, idx) => {
        if (result.status === 'rejected') {
          if (idx === 0) {
            setRequiredDataError(true);
            setRequiredDataErrorMessage(
              `Problems while retrieving namespaces. Error message: ${getResponseError(result.reason)}.`,
            );
          } else {
            setRequiredDataError(true);
            setRequiredDataErrorMessage(
              `Problems while retrieving identity providers. Error message: ${getResponseError(result.reason)}.`,
            );
          }
          return;
        }
            ...
            responseData.map((el) => ({
              ...
              maxPerm:
                roles && roles.includes('admin')
                  ? 7
                  : el.permissions.reduce(
                      (acc, curr) =>
                        curr.username === username ? curr.permission : acc,
                      0,
                    ),
            })),
          );
        ...
        const availableIdentityProvidersTmp = responseData.map((provider) => ({
          value: provider.name,
          label: provider.name,
        }));
        setLdapIdentityProviders(
          responseData
            .filter((provider) => provider.is_ldap_identity_provider === true)
  ...
  const handleInvitationSubmission = async (forceSubmission) => {
    ...
    if (role === 'inviter' && selectedIdentityProvidersAllowed.length === 0) {
      setSubmissionErrorMsg(
        'Please select at least one identity provider that the user is allowed to invite with, or select the "User" role for the invitee.',
      );
      return;
    }
    ...
    if (role !== 'admin') {
      for (let i = 0; i < namespacePermissions.length; i++) {
        if (
          namespacePermissions[i].perm != null &&
          namespacePermissions[i].perm >= 1
        ) {
          invitationSubmissionForm.append(
            'namespace_permissions',
            `${namespacePermissions[i].perm}@${namespacePermissions[i].name}`,
          );
        }
        if (
          namespacePermissions[i].groups != null &&
          namespacePermissions[i].groups.length > 0
        ) {
    ...
    if (availableIdentityProviders.length > 1) {
      ...
      invitationSubmissionForm.append(
        'identity_provider_name',
        availableIdentityProviders[0].value,
      );
    }
    ...
    try {
      ...
      if (err.response && err.response.data && err.response.data.errors) {
        ...
        setSubmissionErrorMsg(
          `An error occurred while creating an invitation code. Error message: ${getResponseError(err)}.`,
        );
      }
      setIsSubmitting(false);
      return;
    ...
    if (!('invitation_token' in inviteReq.data)) {
      setSubmissionErrorMsg(
        'An error occurred while creating an invitation code. Please try again later.',
      );
      setIsSubmitting(false);
      return;
    }
    ...
    <>
      {requiredDataError ? (
        <div className="alert alert-danger mt-3">
          <p>
            <strong>{requiredDataErrorMessage}</strong>
          </p>
        </div>
      ) : (
        <>
          {isLoading ? (
            ...
            <div>
              ...
              >
                ...
                <fieldset disabled={isSubmitting}>
                  ...
                  {role === 'inviter' &&
                    availableIdentityProviders.length === 0 && (
                      <div
                        ...
                        style={{ display: 'block' }}
                      >
                        There are no available identity providers you could give
                        this user to invite with.
                      </div>
                    )}
                  {role !== 'admin' && (
                    <>
                      ...
                      <div className="form-check mt-3 mb-3">
                        <input
                          ...
                          checked={assignQuotas}
                          onChange={(e) => setAssignQuotas(e.target.checked)}
                          id="assignQuotas"
                        />
                        <label
                          className="form-check-label"
                          htmlFor="assignQuotas"
                        >
                          Assign Quotas?
                        </label>
                      </div>
                      {assignQuotas === true && (
                        <UserQuotaSelector setQuotas={setQuotas} />
                      )}
                          ...
                          <>
                            <div className="form-check mt-3 mb-3">
                              <input
                                type="checkbox"
                                className="form-check-input"
                                checked={assignInstances !== true}
                                onChange={(e) =>
                                  setAssignInstances(!e.target.checked)
                                }
                                id="assignInstances"
                              />
                              <label
                                className="form-check-label"
                                htmlFor="assignInstances"
                              >
                                {availableInstances.length === 0 ||
                                (roles && roles.includes('admin'))
                                  ? 'Allowed to use any instance/raw resource requests'
                                  : 'Inherit instances from you'}
                              </label>
                            </div>
                            {assignInstances ? (
                              <>
                  ...
                  {roles && roles.includes('admin') && (
                    <div className="form-check mt-3 mb-3">
                      <input
                        type="checkbox"
                        className="form-check-input"
                        checked={assignLicense !== false}
                        onChange={(e) => setAssignLicense(e.target.checked)}
                        id="assignLicense"
                      />
                      <label
                        className="form-check-label"
                        htmlFor="assignLicense"
                      >
                        Assign License?
                      </label>
                      {assignLicense && (
                  ...
                  {role === 'inviter' &&
                    availableIdentityProviders.length > 1 && (
                      <div className="mb-3">
                        ...
                        <Select
                          ...
                          isSearchable={true}
                          onChange={(selected) =>
                            setSelectedIdentityProvidersAllowed(selected)
                          }
                  ...
                  {ldapIdentityProviders.includes(identityProvider.value) && (
                    <div className="mb-3">
                      <label
                        htmlFor="identityProviderSubject"
                      ...
                      <input
                        ...
                        className={
                          'form-control' +
                          (formErrors.identity_provider_user_subject
                            ? ' is-invalid'
                            : '')
                        }
                        ...
                        onChange={(e) =>
                          setIdentityProviderSubject(e.target.value)
                        }
                        required
                      />
            ...
            <>
              <div className="mt-5">
                Your invitation code is: <code>{invitationCode}</code>
              </div>
          ...
          <Modal
            show={showConfirmNoInstanceDialog}
            onHide={() => setShowConfirmNoInstanceDialog(false)}
          >
            ...
            <Modal.Body>
              <p>
                You are about to invite a user without instances. As a result,
                this user will not be able to solve any jobs until instances are
                assigned to him/her.
              </p>
            </Modal.Body>
            <Modal.Footer>
              <Button
                variant="secondary"
                onClick={() => setShowConfirmNoInstanceDialog(false)}
              >
                Cancel
              </Button>
              <Button
