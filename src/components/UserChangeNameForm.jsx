...
const UserChangeNameForm = () => {
  ...
  const handleChangeUsername = async () => {
    ...
    try {
      await axios.put(`${server}/users/username`, {
        username: user,
      ...
      if (
        ...
        setSubmissionErrorMsg(
          `Some error occurred while trying to change the name of the user: ${user}. Error message: ${getResponseError(err)}.`,
        );
      } else {
    ...
    <>
      {user === 'admin' || !roles.includes('admin') ? (
        <div className="alert alert-danger">
          <p>
            <strong>
              You have no permission to change the username of user: {user}.
            </strong>
          </p>
        </div>
      ) : (
