...
const autoDiscoveryModes = [
  { value: 'manual', label: 'Enter information manually' },
  { value: 'oauth2', label: 'OAuth 2.0 Authorization Server Metadata' },
  { value: 'oidc', label: 'OpenID Connect Discovery 1.0' },
];
const ldapAvailableEncryptionMethods = [
  { value: 'start_tls', label: 'StartTLS' },
  { value: 'simple_tls', label: 'Simple TLS' },
...
const AuthProviderForm = () => {
  ...
  const [autoDiscoveryMode, setAutoDiscoveryMode] = useState(
    autoDiscoveryModes[0],
  );
  const [authorizationEndpoint, setAuthorizationEndpoint] = useState('');
  const [tokenEndpoint, setTokenEndpoint] = useState('');
  const [endSessionEndpoint, setEndSessionEndpoint] = useState('');
  const [deviceAuthorizationEndpoint, setDeviceAuthorizationEndpoint] =
    useState('');
  ...
  const [tokenAuthMethodsSupported, setTokenAuthMethodsSupported] =
    useState('');
  const [requestScopeREADONLY, setRequestScopeREADONLY] = useState('READONLY');
  const [requestScopeCONFIGURATION, setRequestScopeCONFIGURATION] =
    useState('CONFIGURATION');
  const [requestScopeNAMESPACES, setRequestScopeNAMESPACES] =
    useState('NAMESPACES');
  const [requestScopeJOBS, setRequestScopeJOBS] = useState('JOBS');
  const [requestScopeUSERS, setRequestScopeUSERS] = useState('USERS');
  const [requestScopeHYPERCUBE, setRequestScopeHYPERCUBE] =
    useState('HYPERCUBE');
  ...
  const [showRemoveAuthProviderModal, setShowRemoveAuthProviderModal] =
    useState(false);
  ...
  useEffect(() => {
    const fetchAuthProviders = async () => {
      try {
        ...
        setIsLoading(true);
        const [response, responseLdap, responseOidc, responseOAuth] =
          await Promise.all([
        ...
        setAuthProviders(
          response.data.filter(
            (config) => config.is_main_identity_provider !== true,
          ),
        );
  ...
  useEffect(() => {
    const validateIssuerId = async (issuerIDToAssert) => {
      ...
      if (issuerURL.search !== '' || issuerURL.hash !== '') {
        setFormErrors((formErrors) => {
          formErrors.issuer =
            'Issuer URL must not contain query string or fragment components';
          return formErrors;
        });
        setIssuerValid(false);
        return;
      ...
      try {
        ...
        setAutoDiscoveryMode(
          autoDiscoveryModes.filter((mode) => mode.value === 'oidc')[0],
        );
      } catch {
        try {
          await axios.get(
            `${issuerURL.href}/.well-known/oauth-authorization-server`,
          );
          setAutoDiscoveryMode(
            autoDiscoveryModes.filter((mode) => mode.value === 'oauth2')[0],
          );
        } catch {
          setAutoDiscoveryMode(
            autoDiscoveryModes.filter((mode) => mode.value === 'manual')[0],
          );
        }
      }
      setIssuerValid(true);
  ...
  useEffect(() => {
    ...
    if (selectedAuthProvider === '__+add_new') {
      ...
      const providerConfig = authProviders.filter(
        (config) => config.name === selectedAuthProvider,
      );
      ...
      if (providerConfig[0].oauth2 != null || providerConfig[0].oidc != null) {
        ...
        const oAuthProviderConfig = OAuthProviders.filter(
          (config) => config.name === providerConfig[0].name,
        )[0];
        setAutoDiscoveryMode(autoDiscoveryModes[0]);
        setIssuerID(oAuthProviderConfig.issuer);
        setWebuiClientId(oAuthProviderConfig.web_ui_client_id);
        setWebuiClientSecret(
          oAuthProviderConfig.web_ui_client_secret == null
            ? ''
            : oAuthProviderConfig.web_ui_client_secret,
        );
        setDeviceClientId(
          oAuthProviderConfig.device_client_id == null
            ? ''
            : oAuthProviderConfig.device_client_id,
        );
        setDeviceClientSecret(
          oAuthProviderConfig.device_client_secret == null
            ? ''
            : oAuthProviderConfig.device_client_secret,
        );
        setOauthAudience(
          oAuthProviderConfig.override_audience
            ? oAuthProviderConfig.override_audience
        ...
        setEndSessionEndpoint(
          oAuthProviderConfig.end_session_endpoint == null
            ? ''
            : oAuthProviderConfig.end_session_endpoint,
        );
        setDeviceAuthorizationEndpoint(
          oAuthProviderConfig.device_authorization_endpoint == null
            ? ''
            : oAuthProviderConfig.device_authorization_endpoint,
        );
        setJwksUri(oAuthProviderConfig.jwks_uri);
        setTokenAuthMethodsSupported(
          oAuthProviderConfig.token_endpoint_auth_methods_supported.join(','),
        );
        if (providerConfig[0].oidc != null) {
          setExtraClientIds(
            oAuthProviderConfig.extra_client_ids == null
              ? ''
              : oAuthProviderConfig.extra_client_ids.join(','),
          );
          ...
          setOidcUid('');
          setResponseTypesSupported(
            oAuthProviderConfig.response_types_supported.join(','),
          );
          setGrantTypesSupported(
            oAuthProviderConfig.grant_types_supported.join(','),
          );
          setRequestScopeREADONLY(
            oAuthProviderConfig.scopes.filter(
              (scope) => scope.scope === 'READONLY',
            )[0].request_scope,
          );
          setRequestScopeCONFIGURATION(
            oAuthProviderConfig.scopes.filter(
              (scope) => scope.scope === 'CONFIGURATION',
            )[0].request_scope,
          );
          setRequestScopeNAMESPACES(
            oAuthProviderConfig.scopes.filter(
              (scope) => scope.scope === 'NAMESPACES',
            )[0].request_scope,
          );
          setRequestScopeJOBS(
            oAuthProviderConfig.scopes.filter(
              (scope) => scope.scope === 'JOBS',
            )[0].request_scope,
          );
          setRequestScopeUSERS(
            oAuthProviderConfig.scopes.filter(
              (scope) => scope.scope === 'USERS',
            )[0].request_scope,
          );
          setRequestScopeHYPERCUBE(
            oAuthProviderConfig.scopes.filter(
              (scope) => scope.scope === 'HYPERCUBE',
            )[0].request_scope,
          );
          setRequestScopeCLEANUP(
            oAuthProviderConfig.scopes.filter(
              (scope) => scope.scope === 'CLEANUP',
            )[0].request_scope,
          );
          setRequestScopeLICENSES(
            oAuthProviderConfig.scopes.filter(
              (scope) => scope.scope === 'LICENSES',
            )[0].request_scope,
          );
          setRequestScopeUSAGE(
            oAuthProviderConfig.scopes.filter(
        ...
        setLdapBindDN(
          ldapProviderConfig.bind_dn == null ? '' : ldapProviderConfig.bind_dn,
        );
        setLdapPassword(
          ldapProviderConfig.password == null
            ? ''
            : ldapProviderConfig.password,
        );
        setLdapEncryptionMethod(ldapProviderConfig.encryption);
        setLdapVerifyCertificates(
          ldapProviderConfig.verify_certificates !== false,
        );
  ...
  const updateHostname = async (acceptLogout) => {
    ...
    try {
      ...
      setAlertMsg(
        `Problems updating hostname. Error message: ${getResponseError(err)}.`,
      );
    } finally {
  ...
  const handleAuthProviderSubmission = async () => {
    ...
    if (['oidc', 'oauth2'].includes(providerType)) {
      ...
      if (providerType === 'oauth2') {
        ...
        authProviderForm.append(
          ...
          requestScopeCONFIGURATION,
        );
        authProviderForm.append(
          'request_scope_NAMESPACES',
          requestScopeNAMESPACES,
        );
    ...
    try {
      ...
      setAlertMsg(
        `success:Identity provider ${selectedAuthProvider === '__+add_new' ? 'added' : 'updated'} successfully!`,
      );
      ...
      if (err.response && err.response.data && err.response.data.errors) {
        ...
        setSubmissionErrorMsg(
          `Problems updating identity providers. Error message: ${getResponseError(err)}`,
        );
      }
    } finally {
  ...
  return (
    <>
      {isLoading ? (
        ...
        <div className="row">
          <div className="col-md-4 col-12 mt-1 fw-bold"></div>
          <div className="col-md-8 col-12 mt-1 order-2 order-md-1">
            <div className="btn-toolbar mb-2 mb-md-0 float-end"></div>
          </div>
          ...
          <div className="col-md-8 col-12 order-4 mt-1">
            ...
            >
              <div
                ...
                style={{
                  display: submissionErrorMsg !== '' ? 'block' : 'none',
                }}
              >
                {submissionErrorMsg}
              </div>
              <fieldset disabled={isSubmitting}>
                <div className="mt-3 mb-3">
                  ...
                  <input
                    type="text"
                    className={
                      'form-control' + (formErrors.name ? ' is-invalid' : '')
                    }
                    id="providerName"
                ...
                <div className="mt-3 mb-3">
                  ...
                  <Select
                    ...
                    isClearable={false}
                    value={
                      availableProviderTypes.filter(
                        (type) => type.value === providerType,
                      )[0]
                    }
                    isSearchable={true}
                ...
                <div className="mt-3 mb-3">
                  ...
                  <input
                    type="text"
                    className={
                      'form-control' + (formErrors.label ? ' is-invalid' : '')
                    }
                    id="providerLabel"
                    autoComplete="on"
                    required
                ...
                <div className="form-check mt-3">
                  ...
                  />
                  <label className="form-check-label" htmlFor="providerHidden">
                    Hide identity provider on login page?
                  </label>
                  <small
                    id="providerHiddenHelp"
                    className="form-text text-muted"
                  >
                    Will be accessible on <code>/login</code> page via{' '}
                    <code>provider</code> parameter (e.g.{' '}
                    <code>/login?provider=provider1</code> for identity provider
                    with name: <code>provider1</code>).
                  </small>
                </div>
                {['oidc', 'oauth2'].includes(providerType) ? (
                  <>
                    <div className="mt-3 mb-3">
                      ...
                      <small id="issuerHelp" className="form-text text-muted">
                        Using the <i>HTTPS</i> scheme with no query or fragment
                        component
                      </small>
                    </div>
                    <div className="mt-3 mb-3">
                      ...
                      <input
                        ...
                        className={
                          'form-control' +
                          (formErrors.web_ui_client_id ? ' is-invalid' : '')
                        }
                        id="webuiClientId"
                        autoComplete="on"
                        required
                    ...
                    <div className="mt-3 mb-3">
                      <label htmlFor="deviceClientId">
                        Client ID to be used by clients that do not have browser
                        access (optional)
                      </label>
                      <input
                        type="text"
                        className={
                          'form-control' +
                          (formErrors.device_client_id ? ' is-invalid' : '')
                        }
                        id="deviceClientId"
                        autoComplete="on"
                      ...
                      />
                      <div className="invalid-feedback">
                        {formErrors.device_client_id
                          ? formErrors.device_client_id
                          : ''}
                      </div>
                      <small
                        id="deviceClientIdHelp"
                        className="form-text text-muted"
                      >
                        The client must be a public client. Your identity
                        provider must support the{' '}
                        <kbd>urn:ietf:params:oauth:grant-type:device_code</kbd>{' '}
                        grant type and the device authorization endpoint must be
                        provided.
                      </small>
                    </div>
                    <ShowHidePasswordInput
                    ...
                    {providerType === 'oauth2' ? (
                      <>
                        <div className="mt-3 mb-3">
                          ...
                          {serverConfig.hostname !== expectedConfigHostname ? (
                            <div>
                              {hostnameUpdating ? (
                                ...
                                <small>
                                  The Engine configuration does not seem to be
                                  set to the correct hostname (current:{' '}
                                  {serverConfig.hostname}, expected:{' '}
                                  {expectedConfigHostname}). Do you want to
                                  update the hostname now?
                                  <Button
                                    className="btn-update-hostname"
                                    variant="link"
                                    ref={updateHostnameButton}
                                    onClick={() => updateHostname(false)}
                                  >
                                    Update
                                  </Button>
                                </small>
                          ...
                          <small
                            id="oauthAudienceHelp"
                            className="form-text text-muted"
                          >
                            <b>
                              Please do not change the audience unless your
                              identity provider does not allow you to set the
                              audience correctly!
                            </b>
                          </small>
                        </div>
                        <div className="mt-3 mb-3">
                          ...
                          <input
                            ...
                            className={
                              'form-control' +
                              (formErrors.request_scope_READONLY
                                ? ' is-invalid'
                                : '')
                            }
                            ...
                            onChange={(e) =>
                              setRequestScopeREADONLY(e.target.value)
                            }
                          />
                          <div className="invalid-feedback">
                            {formErrors.request_scope_READONLY
                        ...
                        <div className="mt-3 mb-3">
                          ...
                          <input
                            ...
                            className={
                              'form-control' +
                              (formErrors.request_scope_CONFIGURATION
                                ? ' is-invalid'
                                : '')
                            }
                            ...
                            onChange={(e) =>
                              setRequestScopeCONFIGURATION(e.target.value)
                            }
                          />
                          <div className="invalid-feedback">
                            {formErrors.request_scope_CONFIGURATION
                        ...
                        <div className="mt-3 mb-3">
                          ...
                          <input
                            ...
                            className={
                              'form-control' +
                              (formErrors.request_scope_NAMESPACES
                                ? ' is-invalid'
                                : '')
                            }
                            ...
                            onChange={(e) =>
                              setRequestScopeNAMESPACES(e.target.value)
                            }
                          />
                          <div className="invalid-feedback">
                            {formErrors.request_scope_NAMESPACES
                        ...
                        <div className="mt-3 mb-3">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setRequestScopeJOBS(e.target.value)
                            }
                          />
                          <div className="invalid-feedback">
                            {formErrors.request_scope_JOBS
                        ...
                        <div className="mt-3 mb-3">
                          ...
                          <input
                            ...
                            className={
                              'form-control' +
                              (formErrors.request_scope_USERS
                                ? ' is-invalid'
                                : '')
                            }
                            ...
                            onChange={(e) =>
                              setRequestScopeUSERS(e.target.value)
                            }
                          />
                          <div className="invalid-feedback">
                            {formErrors.request_scope_USERS
                        ...
                        <div className="mt-3 mb-3">
                          ...
                          <input
                            ...
                            className={
                              'form-control' +
                              (formErrors.request_scope_HYPERCUBE
                                ? ' is-invalid'
                                : '')
                            }
                            ...
                            onChange={(e) =>
                              setRequestScopeHYPERCUBE(e.target.value)
                            }
                          />
                          <div className="invalid-feedback">
                            {formErrors.request_scope_HYPERCUBE
                        ...
                        <div className="mt-3 mb-3">
                          ...
                          <input
                            ...
                            className={
                              'form-control' +
                              (formErrors.request_scope_CLEANUP
                                ? ' is-invalid'
                                : '')
                            }
                            ...
                            onChange={(e) =>
                              setRequestScopeCLEANUP(e.target.value)
                            }
                          />
                          <div className="invalid-feedback">
                            {formErrors.request_scope_CLEANUP
                        ...
                        <div className="mt-3 mb-3">
                          ...
                          <input
                            ...
                            className={
                              'form-control' +
                              (formErrors.request_scope_LICENSES
                                ? ' is-invalid'
                                : '')
                            }
                            ...
                            onChange={(e) =>
                              setRequestScopeLICENSES(e.target.value)
                            }
                          />
                          <div className="invalid-feedback">
                            {formErrors.request_scope_LICENSES
                        ...
                        <div className="mt-3 mb-3">
                          ...
                          <input
                            ...
                            className={
                              'form-control' +
                              (formErrors.request_scope_USAGE
                                ? ' is-invalid'
                                : '')
                            }
                            ...
                            onChange={(e) =>
                              setRequestScopeUSAGE(e.target.value)
                            }
                          />
                          <div className="invalid-feedback">
                            {formErrors.request_scope_USAGE
                        ...
                        <div className="mt-3 mb-3">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setRequestScopeAUTH(e.target.value)
                            }
                          />
                          <div className="invalid-feedback">
                            {formErrors.request_scope_AUTH
                      ...
                      <>
                        <div className="mt-3 mb-3">
                          <label htmlFor="oidcScopes">
                            The scopes to request from the OP (optional,
                            comma-separated)
                          </label>
                          <input
                            type="text"
                        ...
                        <div className="mt-3 mb-3">
                          <label htmlFor="oidcUid">
                            The claim from the OIDC ID token to use as the
                            user&apos;s unique identifier. The <kbd>sub</kbd>{' '}
                            claim is the default and recommended value.
                          </label>
                          <input
                            type="text"
                            className={
                              'form-control' +
                              (formErrors.uid ? ' is-invalid' : '')
                            }
                            id="oidcUid"
                            autoComplete="on"
                            value={oidcUid}
                        ...
                        <div className="mt-3 mb-3">
                          <label htmlFor="extraClientIDs">
                            Additional client IDs from which the API accepts ID
                            tokens (optional, comma-separated)
                          </label>
                          <input
                            type="text"
                            className={
                              'form-control' +
                              (formErrors.extra_client_ids ? ' is-invalid' : '')
                            }
                            id="extraClientIDs"
                            autoComplete="on"
                      ...
                      <>
                        <div className="mt-3 mb-3">
                          ...
                          <input
                            ...
                            className={
                              'form-control' +
                              (formErrors.authorization_endpoint
                                ? ' is-invalid'
                                : '')
                            }
                        ...
                        <div className="mt-3 mb-3">
                          ...
                          <input
                            ...
                            className={
                              'form-control' +
                              (formErrors.token_endpoint ? ' is-invalid' : '')
                            }
                            id="tokenEndpoint"
                            autoComplete="on"
                            required
                        ...
                        <div className="mt-3 mb-3">
                          ...
                          <input
                            ...
                            className={
                              'form-control' +
                              (formErrors.device_authorization_endpoint
                                ? ' is-invalid'
                                : '')
                            }
                            ...
                            onChange={(e) =>
                              setDeviceAuthorizationEndpoint(e.target.value)
                            }
                          />
                          <div className="invalid-feedback">
                            {formErrors.device_authorization_endpoint
                        ...
                        <div className="mt-3 mb-3">
                          ...
                          <input
                            ...
                            className={
                              'form-control' +
                              (formErrors.jwks_uri ? ' is-invalid' : '')
                            }
                            id="jwksUri"
                            autoComplete="on"
                            value={jwksUri}
                        ...
                        <div className="mt-3 mb-3">
                          ...
                          <input
                            ...
                            className={
                              'form-control' +
                              (formErrors.end_session_endpoint
                                ? ' is-invalid'
                                : '')
                            }
                            ...
                            onChange={(e) =>
                              setEndSessionEndpoint(e.target.value)
                            }
                          />
                          <div className="invalid-feedback">
                            {formErrors.end_session_endpoint
                        ...
                        <div className="mt-3 mb-3">
                          ...
                          <input
                            ...
                            className={
                              'form-control' +
                              (formErrors.token_endpoint_auth_methods_supported
                                ? ' is-invalid'
                                : '')
                            }
                            ...
                            onChange={(e) =>
                              setTokenAuthMethodsSupported(e.target.value)
                            }
                          />
                          <div className="invalid-feedback">
                            {formErrors.token_endpoint_auth_methods_supported
                      ...
                      <>
                        <div className="mt-3 mb-3">
                          ...
                          <input
                            ...
                            className={
                              'form-control' +
                              (formErrors.response_types_supported
                                ? ' is-invalid'
                                : '')
                            }
                            ...
                            onChange={(e) =>
                              setResponseTypesSupported(e.target.value)
                            }
                          />
                          <div className="invalid-feedback">
                            {formErrors.response_types_supported
                        ...
                        <div className="mt-3 mb-3">
                          <label htmlFor="grantTypesSupported">
                            List of grant types that the provider supports
                            (comma-separated)
                          </label>
                          <input
                            type="text"
                            className={
                              'form-control' +
                              (formErrors.grant_types_supported
                                ? ' is-invalid'
                                : '')
                            }
                            ...
                            onChange={(e) =>
                              setGrantTypesSupported(e.target.value)
                            }
                          />
                          <div className="invalid-feedback">
                            {formErrors.grant_types_supported
                  ...
                  <>
                    <div className="mt-3 mb-3">
                      <label htmlFor="ldapHost">Host of the LDAP server</label>
                      <input
                        type="text"
                        className={
                          'form-control' +
                          (formErrors.host ? ' is-invalid' : '')
                        }
                        id="ldapHost"
                    ...
                    <div className="mt-3 mb-3">
                      <label htmlFor="ldapPort">Port of the LDAP server</label>
                      <input
                        type="number"
                        className={
                          'form-control' +
                          (formErrors.port ? ' is-invalid' : '')
                        }
                        id="ldapPort"
                      ...
                      <small id="ldapPortHelp" className="form-text text-muted">
                        Examples: 389 (Plain or StartTLS) and 636 (for Simple
                        TLS)
                      </small>
                    </div>
                    <div className="form-check mt-3">
                      <input
                        ...
                        onChange={(e) =>
                          setLdapActiveDirectory(e.target.checked)
                        }
                      ...
                      <label
                        ...
                        htmlFor="ldapActiveDirectory"
                      >
                        Is LDAP server an Active Directory (AD) LDAP server?
                      </label>
                      <small
                        id="ldapActiveDirectoryHelp"
                        className="form-text text-muted"
                      >
                    ...
                    <div className="mt-3 mb-3">
                      ...
                      <input
                        type="text"
                        className={
                          'form-control' + (formErrors.uid ? ' is-invalid' : '')
                        }
                        id="ldapUid"
                        autoComplete="on"
                      ...
                      <small id="ldapUidHelp" className="form-text text-muted">
                        Examples: <code>sAMAccountName</code> for Active
                        Directory, <code>uid</code> for OpenLDAP. This will be
                        appended to <code>user_filter</code> with an <i>&</i>{' '}
                        clause.
                      </small>
                    </div>
                    <div className="mt-3 mb-3">
                      <label htmlFor="ldapBindDN">
                        Full distinguished name (DN) of the user who is used for
                        binding
                      </label>
                      <input
                        type="text"
                      ...
                      >
                        ...
                        <code>uid=admin,ou=users,dc=example,dc=org</code>.
                        Example for Active Directory: <code>EXAMPLE\admin</code>
                        .
                      </small>
                    </div>
                    {ldapBindDN !== '' ? (
                    ...
                    <div className="mt-3 mb-3">
                      ...
                      <input
                        ...
                        className={
                          'form-control' +
                          (formErrors.user_filter ? ' is-invalid' : '')
                        }
                        id="ldapUserFilter"
                        autoComplete="on"
                      ...
                      <small
                        id="ldapUserFilterHelp"
                        className="form-text text-muted"
                      >
                        Examples: <code>(objectClass=User)</code> or{' '}
                        <code>
                          (memberOf=cn=gams-engine,ou=groups,dc=example,dc=com)
                        </code>{' '}
                        or{' '}
                        <code>
                          &((objetClass=User),(employeeType=developer))
                        </code>
                        .
                      </small>
                    </div>
                    <div className="mt-3 mb-3">
                      ...
                      <Select
                        ...
                        isClearable={false}
                        value={
                          ldapAvailableEncryptionMethods.filter(
                            (type) => type.value === ldapEncryptionMethod,
                          )[0]
                        }
                        isSearchable={true}
                        onChange={(selected) =>
                          setLdapEncryptionMethod(selected.value)
                        }
                    ...
                    {ldapEncryptionMethod !== 'plain' ? (
                      <div className="form-check mt-3">
                        <input
                          ...
                          onChange={(e) =>
                            setLdapVerifyCertificates(e.target.checked)
                          }
                        ...
                        <label
                          className="form-check-label"
                          htmlFor="ldapVerifyCertificates"
                        >
                          Verify TLS certificate signature?
                        </label>
                        <small
                          id="ldapVerifyCertificatesHelp"
                          className="form-text text-muted"
                        >
                          Please deactivate this function only if you know what
                          you are doing, as this is a critical security feature!
                        </small>
                      </div>
                    ) : (
                      <></>
              ...
              <div className="mt-3">
                <SubmitButton isSubmitting={isSubmitting}>
                  {selectedAuthProvider === '__+add_new'
                    ? 'Add Provider'
                    : 'Update'}
                </SubmitButton>
              </div>
      ...
      />
      <Modal
        show={showUpdateHostnameModal}
        onHide={() => setShowUpdateHostnameModal(false)}
      >
        <Modal.Header closeButton>
          <Modal.Title>Please Confirm</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          Are you sure you want to update the hostname to:{' '}
          <code>{expectedConfigHostname}</code>? This will invalidate all JWT
          tokens and consequently log out all users (including yourself).
        </Modal.Body>
        <Modal.Footer>
          <Button
            variant="secondary"
            onClick={() => setShowUpdateHostnameModal(false)}
          >
            Cancel
          </Button>
