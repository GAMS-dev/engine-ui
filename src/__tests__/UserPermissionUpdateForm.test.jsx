...
describe('UserPermissionUpdateForm', () => {
  ...
  it('changes role to admin correctly', async () => {
    ...
    const roleSelectorEl = screen.getByRole('combobox', {
      name: 'Specify a role for the user',
    });
    await userEvent.selectOptions(roleSelectorEl, 'Admin');
    await user.click(
      screen.getByRole('button', { name: 'Update Permissions' }),
    );
    expect(axios.put).toBeCalledWith('testserver/users/role', {
      roles: ['admin'],
      username: 'user1',
    });
  ...
  it('changes role from user to inviter with different provider correctly', async () => {
    ...
    axios.put.mockImplementation((url, data) => {
      switch (url) {
        case 'testserver/users/role': {
          if (
            JSON.stringify(data) ===
            JSON.stringify({ username: 'user1', roles: ['inviter'] })
          ) {
    ...
    await waitFor(() =>
      screen.findByText(/Identity providers user is allowed to invite with/),
    );
    await user.click(
      screen.getByRole('button', { name: 'Remove gams_engine' }),
    );
    await user.click(
      screen.getByRole('button', { name: 'Update Permissions' }),
    );
  ...
  it('fails if changes role from user to inviter with no provider specified', async () => {
    ...
    await user.click(
      screen.getByRole('button', { name: 'Remove some_provider' }),
    );
    await user.click(
      screen.getByRole('button', { name: 'Update Permissions' }),
    );
  ...
  it('handles errors while retrieving both fail', async () => {
    ...
    await waitFor(() =>
      screen.findByText(
        /Problems while retrieving user roles. Error message: undefined./,
      ),
    );
  });
  ...
  it('handles errors while retrieving namespace', async () => {
    ...
    await waitFor(() =>
      screen.findByText(
        /Problems while retrieving namespaces. Error message: not found./,
      ),
    );
  });
  ...
  it('handles errors while retrieving identity provider', async () => {
    ...
    await waitFor(() =>
      screen.findByText(
        /Problems while retrieving identity providers. Error message: not found./,
      ),
    );
  });
  ...
  it('handles errors while submitting role', async () => {
    ...
    await waitFor(() =>
      screen.findByText(
        /An error occurred while updating user roles. Error message: undefined./,
      ),
    );
  });
  ...
  it('handles errors while submitting identity providers', async () => {
    ...
    await waitFor(() =>
      screen.findByText(
        /An error occurred while updating available identity providers. Error message: undefined./,
      ),
    );
  });
  ...
  it('handles errors while submitting user permissions', async () => {
    ...
    await waitFor(() =>
      screen.findByText(
        /An error occurred while updating user permissions. Error message: undefined./,
      ),
    );
  });
});
