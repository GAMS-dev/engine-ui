...
export const AllProvidersWrapperDefault = ({ children, options = {} }) => {
  ...
  const quotaFormattingFn =
    quotaUnit === '$'
      ...
      : (val) =>
          `${new Intl.NumberFormat('en-US', { style: 'decimal' }).format(val / 3600)}${Number.isFinite(val) ? 'h' : ''}`;
  ...
  const use_brokerv2 = options.use_brokerv2 ?? false;
        ...
        <Route
          ...
          element={
            <ServerConfigContext.Provider
              value={[serverConfig, setServerConfig]}
            >
              ...
              >
                ...
                >
                  <AuthContext.Provider
                    value={
                      login
                        ? [{ username, roles, server }, setLogin]
                        : [login, setLogin]
                    }
                  >
                    {children}
