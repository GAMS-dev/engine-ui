...
describe('UserQuotaUpdateForm', () => {
  ...
  it('displays everything correctly', async () => {
    ...
    const parallelSpinButton = screen.getByRole('spinbutton', {
      name: 'Parallel Quota (in ¢/s) Set to max',
    });
    const volumeSpinButton = screen.getByRole('spinbutton', {
      name: 'Volume Quota (in $) Set to max',
    });
    const diskSpinButton = screen.getByRole('spinbutton', {
      name: 'Disk Space Quota (in MB) Set to max',
    });
  ...
  it('set to max buttons work', async () => {
    ...
    const setMaxButtons = screen.getAllByRole('button', {
      name: /set to max/i,
    });
    ...
    const parallelInput = screen.getByRole('spinbutton', {
      name: /parallel quota/i,
    });
  ...
  it('submits correctly if everything is updated', async () => {
    ...
    await waitFor(() => {
      expect(axios.put).toBeCalledWith('testserver/usage/quota', null, {
        params: {
          username: 'user1',
          disk_quota: 100000,
          parallel_quota: 2,
  ...
  it('submits correctly if everything is deleted', async () => {
    ...
    expect(axios.delete).toBeCalledWith('testserver/usage/quota', {
      params: { field: 'parallel_quota', username: 'user1' },
    });
    expect(axios.delete).toBeCalledWith('testserver/usage/quota', {
      params: { field: 'volume_quota', username: 'user1' },
    });
    expect(axios.delete).toBeCalledWith('testserver/usage/quota', {
      params: { field: 'disk_quota', username: 'user1' },
    });
  });
  ...
  it('selector does not accept invalid input', async () => {
    ...
    fireEvent.change(parallelSpinButton, {
      target: { class: 'form-control is-invalid' },
    });
    fireEvent.change(volumeSpinButton, {
      target: { class: 'form-control is-invalid' },
    });
    fireEvent.change(diskSpinButton, {
      target: { class: 'form-control is-invalid' },
    });
  ...
  it('handles errors while retrieving user quotas', async () => {
    ...
    await waitFor(() =>
      screen.findByText(
        /Problems while retrieving user quotas. Error message: get failed./,
      ),
    );
  });
  ...
  it('handles errors updating quota', async () => {
    ...
    await waitFor(() =>
      screen.findByText(
        /An error occurred while updating user quotas. Error message: put failed./,
      ),
    );
  });
