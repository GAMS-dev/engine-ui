...
describe('UserChangePassForm', () => {
  ...
  it('sends the correct put request', async () => {
    ...
    (await user.click(screen.getByRole('button')),
      expect(axios.put).toBeCalledWith('testserver/users/', {
        username: 'user1',
        password: 'new', // pragma: allowlist secret
      }));
  });
  ...
  it('handles errors correctly', async () => {
    ...
    await waitFor(() =>
      screen.findByText(
        /Some error occurred while trying to change the password. Error message: something else than the password policy went wrong./,
      ),
    );
  });
  ...
  it('handles errors from the password policy correctly', async () => {
    ...
    expect(
      screen.queryByText(
        /Some error occurred while trying to changing the password. Error message:/,
      ),
    ).toBeNull();
  });
