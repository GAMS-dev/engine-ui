...
const DummyConsumer = () => {
    ...
    return (
        <div>
            ...
            <span data-testid="brokerv2">{serverInfo.use_brokerv2 ? 'true' : 'false'}</span>
...
describe('ServerInfoProvider', () => {
    ...
    it('fetches server info when no valid cache exists', async () => {
        axios.get.mockImplementation((url) => {
            if (url.includes('/version')) return Promise.resolve({ data: { version: 'x.x.x' } });
            if (url.includes('/configuration')) return Promise.resolve({ data: { use_brokerv2: true } });
        ...
        await waitFor(() => {
            expect(axios.get).toHaveBeenCalledWith(expect.stringContaining('/version'));
            expect(axios.get).toHaveBeenCalledWith(expect.stringContaining('/configuration'));
        });
    ...
    it('defaults use_brokerv2 to false if the endpoint omits it', async () => {
        axios.get.mockImplementation((url) => {
            if (url.includes('/version')) return Promise.resolve({ data: { version: 'x.x.x' } });
            if (url.includes('/configuration')) return Promise.resolve({ data: { some_other_key: true } }); // Missing use_brokerv2
            return Promise.reject(new Error('not found'));
        });
