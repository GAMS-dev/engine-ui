...
describe('test UpdatePasswordPolicyButton', () => {
  ...
  beforeEach(() => {
    ...
    axios.get.mockImplementation(() =>
      Promise.resolve({ status: 200, data: passwordPolicy }),
    );
  });
  ...
  it('display default passwordPolicy on first use', async () => {
    ...
    expect(
      screen.getByRole('spinbutton', { name: 'Minimum password length:' })
        .value,
    ).toEqual('10');
    expect(
      screen.getByRole('checkbox', {
        name: 'Include at least one uppercase letter?',
      }).checked,
    ).toEqual(false);
    expect(
      screen.getByRole('checkbox', {
        name: 'Include at least one lowercase letter?',
      }).checked,
    ).toEqual(false);
    expect(
      screen.getByRole('checkbox', { name: 'Include at least one number?' })
        .checked,
    ).toEqual(false);
    expect(
      screen.getByRole('checkbox', {
    ...
    expect(
      screen.getByRole('checkbox', {
        name: 'Check if the password is commonly used?',
      }).checked,
    ).toEqual(false);

    expect(screen.getByRole('button', { name: 'Cancel' })).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: 'Update', type: 'submit' }),
    ).toBeInTheDocument();
  });
  ...
  it('password policy updates correctly', async () => {
    ...
    expect(axios.put).toBeCalledWith('testserver/auth/password-policy', {
      min_password_length: 10,
      must_include_uppercase: false,
      must_include_lowercase: false,
      must_include_number: false,
  ...
  it('handles error while updating the password policy correctly (put)', async () => {
    ...
    await waitFor(() =>
      screen.getByText(
        "Couldn't set new password policy. Error message: Test error.",
      ),
    );
  });
