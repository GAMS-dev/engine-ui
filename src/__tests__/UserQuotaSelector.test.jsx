...
describe('UserQuotaSelector', () => {
  ...
  it('displays the given quota data correctly for user1', async () => {
    ...
    expect(
      screen.getByRole('spinbutton', { name: /parallel quota/i }),
    ).toHaveValue(1);
    expect(
      screen.getByRole('spinbutton', { name: /volume quota/i }),
    ).toHaveValue(36);
    expect(
      screen.getByRole('spinbutton', { name: /disk space quota/i }),
    ).toHaveValue(1);
  });
  ...
  it('calculates max quota based on the inviter (user1) when editing user2', async () => {
    ...
    const diskInput = screen.getByRole('spinbutton', {
      name: /disk space quota/i,
    });
  ...
  it('shows 0/negative remaining if the Inviter (user1) has used all their quota', async () => {
    ...
    const volumeHint = screen.getByText((content, element) => {
      return (
        ...
        (/remaining:\s*0(\.00)?/.test(content) ||
          /remaining:\s*-\$/.test(content)) &&
